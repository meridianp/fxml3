...
check_prerequisites() {
    ...
    # Export password without quotes to avoid issues with special characters
...
create_database() {
    ...
    local db_exists=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -tAc "SELECT 1 FROM pg_database WHERE datname='$DB_NAME';")
