...
class DataPipeline:
    ...
    def process(
        ...
        if self.resampled_timeframe is not None:
            try:
                ...
                metadata["processing_steps"].append(
                    f"resample_{self.resampled_timeframe}"
                )
        ...
        if self.add_indicators:
            try:
                logger.info(
                    f"Adding technical indicators: {', '.join(self.indicators)}"
                )
        ...
        logger.info(
            f"Processing complete. Original shape: {metadata['original_shape']}, Final shape: {metadata['final_shape']}"
        )
