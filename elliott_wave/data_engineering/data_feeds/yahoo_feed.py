...
class YahooDataFeed(DataFeed):
    ...
    def _load_from_cache(
        ...
        cache_filename = (
            f"{std_symbol}_{std_timeframe}_{start_date_str}_{end_date_str}.csv"
        )
        ...
        if os.path.exists(cache_path):
            ...
            if mod_date.date() == datetime.now().date() and (
                end_date is None
                or (
                    isinstance(end_date, datetime)
                    and end_date.date() >= datetime.now().date()
                )
            ):
                try:
    ...
    def get_available_symbols(self) -> List[str]:
        ...
        raise NotImplementedError(
            "Yahoo Finance doesn't support listing all available symbols"
        )
