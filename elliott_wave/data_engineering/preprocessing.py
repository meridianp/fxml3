...
def normalize_data(
    ...
    if method == "min_max":
        ...
        for col in available_columns:
            ...
            if std_val == 0:
                logger.warning(
                    f"Standard deviation is 0 for '{col}', skipping normalization"
                )
                continue
...
def denormalize_data(df: pd.DataFrame, params: Dict) -> pd.DataFrame:
    ...
    if method == "min_max":
        ...
        price_columns = ["open", "high", "low", "close"]
        available_columns = [
            col for col in price_columns if col in denormalized_df.columns
        ]

        for col in available_columns:
            denormalized_df[col] = (denormalized_df[col] - range_min) / (
                range_max - range_min
    ...
    try:
        ...
        for indicator in indicators:
            if indicator == "sma":
                ...
                for period in periods:
                    result_df[f"rsi_{period}"] = ta.rsi(
                        result_df["close"], length=period
                    )
...
def resample_data(
    ...
    for col in agg_dict.keys():
        if col not in df.columns:
            logger.warning(
                f"Column '{col}' specified in aggregation dictionary not found in DataFrame"
            )
