...
def evaluate_agent(
    ...
    if all_episode_returns:
        ...
        sharpe_ratio = np.mean(all_episode_returns) / (
            np.std(all_episode_returns) + 1e-8
        )
...
def plot_evaluation_results(
    ...
    for i, balance_history in enumerate(metrics["all_balances"]):
        axes[0, 0].plot(
            balance_history, alpha=0.3, label=f"Episode {i+1}" if i < 5 else None
        )
...
def compare_agents(
    ...
    for agent_name, agent in agents.items():
        ...
        mean_reward, rewards, metrics = evaluate_agent(
            env=env, agent=agent, num_episodes=num_episodes, render=False
        )
    ...
    print("\nAgent Comparison:")
    print(
        f"{'Agent':<20} {'Mean Reward':<15} {'Mean Final Balance':<20} {'Sharpe Ratio':<15} {'Max Drawdown':<15}"
    )
    print("-" * 85)
