...
class ResultVisualizer:
        ...
        if "overall" in metrics:
            ...
            if "by_pattern_type" in metrics:
                if "impulse" in metrics["by_pattern_type"]:
                    metrics_to_plot["Impulse Accuracy"] = (
                        metrics["by_pattern_type"]["impulse"].get("accuracy", 0) * 100
                    )

                if "corrective" in metrics["by_pattern_type"]:
                    metrics_to_plot["Corrective Accuracy"] = (
                        metrics["by_pattern_type"]["corrective"].get("accuracy", 0)
                        * 100
                    )
        else:
            # Use root metrics
    ...
    def plot_advanced_equity_curve(
        ...
        if monte_carlo_results:
            fig, (ax1, ax2, ax3) = plt.subplots(
                3, 1, figsize=figsize, gridspec_kw={"height_ratios": [3, 1, 2]}
            )
        else:
        ...
        ax2.fill_between(
            trades, drawdown, 0, alpha=0.3, color="red", label="Drawdown %"
        )
        ...
        if trade_outcomes:
            ...
            for i, outcome in enumerate(trade_outcomes):
                if "total_cost" in outcome:
                    trade_indices.append(
                        i + 1
                    )  # +1 because equity curve starts with initial capital
            ...
            if trade_costs:
                ...
                cost_ax.set_ylim(
                    0, max(trade_costs) * 3
                )  # Scale to make bars visible but not dominant
        ...
        if monte_carlo_results:
            ...
            initial_capital = monte_carlo_results.get(
            ...
            ax3.plot(
                ...
                color="red",
                alpha=0.7,
                label=f"Worst Case: ${worst_case:,.2f}",
            )
            ax3.plot(
                x,
                expected_curve,
                "-",
                color="blue",
                label=f"Expected: ${expected_case:,.2f}",
            )
            ax3.plot(
                x,
                best_curve,
                ...
                color="green",
                alpha=0.7,
                label=f"Best Case: ${best_case:,.2f}",
            )
    ...
    def plot_equity_curve(
        ...
        df = pd.DataFrame({"Equity": equity_curve, "Drawdown": drawdown})
    ...
    def plot_interactive_results(
        ...
        if "by_pattern_type" in metrics:
            ...
            if "corrective" in metrics["by_pattern_type"]:
                ...
                pattern_values.append(
                    metrics["by_pattern_type"]["corrective"].get("accuracy", 0) * 100
                )
        ...
        if "windows" in backtest_results:
            ...
            accuracies = [
                c / t if t > 0 else 0 for c, t in zip(correct_counts, total_counts)
            ]
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    text=[
                        f"Window {i}: {a:.2f}"
                        for i, a in zip(window_indices, accuracies)
                    ],
    ...
    def plot_cross_market_validation(
        ...
        if (
            not cross_market_results
            or "market_results" not in cross_market_results
            or not cross_market_results["market_results"]
        ):
            ...
            ax.text(
                0.5,
                0.5,
                "No cross-market validation data available",
                ha="center",
                va="center",
                fontsize=14,
            )
        ...
        if "consistency_metrics" in cross_market_results:
            ...
            if metrics:
                ...
                consistency_scores = [
                    consistency[m].get("consistency_score", 0) * 100 for m in metrics
                ]

                y_pos = np.arange(len(metric_names))
                bars = ax2.barh(
                    y_pos, consistency_scores, align="center", alpha=0.7, color="purple"
                )
        ...
        if (
            "correlation_matrix" in cross_market_results
            and cross_market_results["correlation_matrix"]
        ):
    ...
    def plot_walk_forward_analysis(
        ...
        if (
            not wfa_results
            or "fold_results" not in wfa_results
            or not wfa_results["fold_results"]
        ):
            # Create empty figure if no data
            fig, ax = plt.subplots(figsize=figsize)
            ax.text(
                0.5,
                0.5,
                "No walk-forward analysis data available",
                ha="center",
                va="center",
                fontsize=14,
            )
        ...
        if "equity_curve" in wfa_results:
            ...
            for fold_idx, fold in enumerate(fold_results):
                ...
                if (
                    "validation_results" in fold
                    and "actual_outcomes" in fold["validation_results"]
                ):
            ...
            for i, boundary in enumerate(fold_boundaries):
                if boundary < len(equity_curve):
                    ...
                    ax1.text(
                        boundary,
                        min(equity_curve)
                        + (max(equity_curve) - min(equity_curve)) * 0.05,
                        f"Fold {i+1}",
                        rotation=90,
                        verticalalignment="bottom",
                        alpha=0.7,
        ...
        for fold in fold_results:
            ...
            if "capital_after_fold" in fold:
                initial_capital = wfa_results.get("performance_metrics", {}).get(
                    "initial_capital", 10000
                )
                capital_before = (
                    initial_capital
                    if fold.get("fold", 0) == 1
                    else fold_results[fold.get("fold", 0) - 2].get(
                        "capital_after_fold", initial_capital
                    )
                )
        ...
        if fold_numbers and fold_returns:
            ...
            for i, v in enumerate(fold_returns):
                ax2.text(
                    i, v + (v > 0) * 0.5 - (v <= 0) * 1.5, f"{v:.1f}%", ha="center"
                )
        ...
        if "parameter_stability" in wfa_results:
            ...
            param_names = [
                p for p in param_stability.keys() if p != "overall_stability"
            ]
            stability_scores = [
                param_stability[p].get("stability_score", 0) * 100 for p in param_names
            ]
            ...
            if param_names and stability_scores:
                ...
                bars = ax3.barh(
                    y_pos, stability_scores, align="center", alpha=0.7, color="purple"
                )
    ...
    def plot_prediction_examples(
        ...
        fig = make_subplots(
            ...
            cols=cols,
            subplot_titles=[
                f"Example {i+1}: {p['pattern_type'].capitalize()} Pattern"
        ...
        for i, (pred, actual) in enumerate(paired_examples):
            ...
            fig.add_shape(
                ...
                x1=(
                    price_data.index[future_end - 1]
                    if future_end < len(price_data)
                    else price_data.index[-1]
                ),
                y1=pred["target_price"],
                line=dict(
                    color="#2ca02c" if actual["correct"] else "#d62728",
                    width=2,
                    dash="dot",
                ),
                row=row,
            ...
            fig.add_annotation(
                x=price_data.index[
                    min(end_idx + horizon_length // 2, len(price_data) - 1)
                ],
