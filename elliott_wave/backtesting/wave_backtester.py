...
class WaveBacktester:
    ...
    def run_rolling_window_backtest(
        ...
        for start_idx in range(
            0, len(data) - window_size - prediction_horizon, step_size
        ):
            # Extract window data
        ...
        if stats["predictions_made"] > 0:
            stats["prediction_accuracy"] = (
                stats["correct_predictions"] / stats["predictions_made"]
            )
            stats["incorrect_predictions"] = (
                stats["predictions_made"] - stats["correct_predictions"]
            )
    ...
    def _analyze_window_and_predict(
        ...
        impulse_pattern_keys = [
            k
            for k in wave_points.keys()
            if k.startswith("impulse_") and k.endswith("_end")
        ]
        corrective_pattern_keys = [
            k
            for k in wave_points.keys()
            if k.startswith("corrective_") and k.endswith("_end")
        ]

        # Count patterns
        window_stats["impulse_patterns"] = (
            len(impulse_pattern_keys) // 5
        )  # 5 waves per impulse pattern
        window_stats["corrective_patterns"] = (
            len(corrective_pattern_keys) // 3
        )  # 3 waves per corrective pattern
    ...
    def _generate_predictions(
        ...
        impulse_wave5_end_keys = [
            k
            for k in wave_points.keys()
            if k.startswith("impulse_") and k.endswith("5_end")
        ]
        ...
        corrective_waveC_end_keys = [
            k
            for k in wave_points.keys()
            if k.startswith("corrective_") and k.endswith("C_end")
        ]
    ...
    def _validate_prediction(
        ...
        if not future_data.empty:
            ...
            spread_cost_in_money = (
                entry_spread * position_size if direction == "up" else 0
            )
            ...
            if not target_reached and not stop_hit and not future_data.empty:
                ...
                exit_slippage = self._calculate_slippage(
                    last_price, "down" if direction == "up" else "up"
                )
    ...
    def _calculate_spread(self, price_data: pd.Series, reference_price: float) -> float:
        ...
        if self.spread_model == "variable":
            ...
            if "high" in price_data and "low" in price_data:
                ...
                volatility_factor = (
                    params["volatility_factor"] * volatility * 10000
    ...
    def run_walk_forward_analysis(
        ...
        fold_size = (
            (len(df) - (window_size + validation_size)) // (n_folds - 1)
            if n_folds > 1
            else len(df)
        )
        ...
        for fold in range(n_folds):
            ...
            if (
                "actual_outcomes" in backtest_result
                and backtest_result["actual_outcomes"]
            ):
    ...
    def _optimize_parameters(self, train_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        for peak, trough, wave, overlap in product(
            ...
            impulse_patterns = sum(
                1
                for k in wave_points.keys()
                if k.startswith("impulse_") and k.endswith("_end")
            )
            corrective_patterns = sum(
                1
                for k in wave_points.keys()
                if k.startswith("corrective_") and k.endswith("_end")
            )
    ...
    def _calculate_parameter_stability(
        self, parameters_by_fold: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for param_name in [
            ...
            param_values[param_name] = [
                p.get(param_name, None) for p in parameters_by_fold if param_name in p
            ]
        ...
        if stability_metrics:
            overall_stability = np.mean(
                [m["stability_score"] for m in stability_metrics.values()]
            )
            ...
            try:
                ...
                if (
                    ...
                    if "equity_curve" in performance_metrics:
                        ...
                        if (
                            len(equity_curve) > 1
                        ):  # Need at least two points for returns
    ...
    def _calculate_cross_market_consistency(
        self, market_results: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        if win_rates:
            consistency["win_rate"] = {
                ...
                "cv": (
                    np.std(win_rates) / np.mean(win_rates)
                ...
                "max": max(win_rates),
                "consistency_score": (
                    1.0 - (np.std(win_rates) / np.mean(win_rates))
                    if np.mean(win_rates) > 0
                    else 0
                ),
            }
        ...
        if profit_factors:
            consistency["profit_factor"] = {
                ...
                "cv": (
                    np.std(profit_factors) / np.mean(profit_factors)
                    if np.mean(profit_factors) > 0
                    else 0
                ),
                "min": min(profit_factors),
                "max": max(profit_factors),
                "consistency_score": (
                    1.0 - (np.std(profit_factors) / np.mean(profit_factors))
                    if np.mean(profit_factors) > 0
                    else 0
                ),
            }
        ...
        if returns:
            consistency["return_pct"] = {
                ...
                "max": max(returns),
                "consistency_score": (
                    1.0 - (np.std(returns) / abs(np.mean(returns)))
                    if np.mean(returns) != 0
                    else 0
                ),
            }
        ...
        if drawdowns:
            consistency["max_drawdown"] = {
                ...
                "cv": (
                    np.std(drawdowns) / np.mean(drawdowns)
                ...
                "max": max(drawdowns),
                "consistency_score": (
                    1.0 - (np.std(drawdowns) / np.mean(drawdowns))
                    if np.mean(drawdowns) > 0
                    else 0
                ),
            }
    ...
    def run_monte_carlo_simulation(
        ...
        mc_results = {
            ...
            "profit_factor": (
                -np.sum(
                    final_capitals[final_capitals > initial_capital] - initial_capital
                )
                / np.sum(
                    final_capitals[final_capitals < initial_capital] - initial_capital
                )
    ...
    def analyze_results(self) -> Dict[str, Any]:
        ...
        patterns_detected = self.results.get(
            "impulse_patterns_detected", 0
        ...
        if self.predictions and self.actual_outcomes:
            ...
            for pred, actual in zip(self.predictions, self.actual_outcomes):
                ...
                if "risk_reward_ratio" in actual and actual[
                    "risk_reward_ratio"
                ] != float("inf"):
            ...
            if impulse_total > 0:
                analysis["success_by_pattern_type"]["impulse"] = (
                    impulse_correct / impulse_total
                )

            if corrective_total > 0:
                analysis["success_by_pattern_type"]["corrective"] = (
                    corrective_correct / corrective_total
                )
