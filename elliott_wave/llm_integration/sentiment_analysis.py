    ...
    def fetch_news(
        ...
        if self.cache_dir:
            cache_file = os.path.join(
                self.cache_dir, f"yahoo_news_{symbol}_{days_back}d_{limit}.json"
            )
...
class IBKRNewsFetcher(NewsFetcher):
    ...
    def fetch_news(
        ...
        if self.cache_dir:
            cache_file = os.path.join(
                self.cache_dir, f"ibkr_news_{symbol}_{days_back}d_{limit}.json"
            )
                ...
                if hasattr(headline, "articleId"):
                    try:
                        article = ib.reqNewsArticle(
                            headline.provider, headline.articleId
                        )
    ...
    def analyze_sentiment(
        ...
        try:
            ...
            if self.cache_dir:
                try:
                    with open(
                        os.path.join(self.cache_dir, f"sentiment_{cache_key}.json"), "w"
                    ) as f:
...
class SentimentAggregator:
    ...
    def create_sentiment_timeseries(
        ...
        for item in sentiment_data:
            ...
            weighted_sentiment = (
                normalized_intensity * (relevance / 10) * (confidence / 10)
            )
...
class MarketSentimentAnalyzer:
    ...
    def __init__(
        ...
        if use_yahoo:
            ...
            self.news_sources.append(YahooFinanceNewsFetcher(cache_dir=yahoo_cache_dir))

        if use_ibkr:
            ibkr_cache_dir = os.path.join(self.cache_dir, "ibkr")
            self.news_sources.append(IBKRNewsFetcher(cache_dir=ibkr_cache_dir))
    ...
    def analyze_market_sentiment(
        ...
        result = {
            ...
            "data": {
                ...
                "timeframe": timeframe,
                "timeseries": (
                    indicators_df.reset_index().to_dict(orient="records")
                    if not indicators_df.empty
                    else []
                ),
            },
    ...
    def validate_wave_with_sentiment(
        ...
        if pattern_type == "impulse":
            ...
            validation["explanation"] = (
                f"Wave {wave_count} ({pattern_type}) typically shows {expected_sentiment} sentiment. Current sentiment is {overall_sentiment} with score {sentiment_score:.2f}."
            )
        ...
        if "timeseries" in sentiment_data.get("data", {}):
            ...
            for entry in sentiment_data["data"]["timeseries"]:
                ...
                if start <= timestamp and (
                    not end_date or timestamp <= datetime.fromisoformat(end_date)
                ):
