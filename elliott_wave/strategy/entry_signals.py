...
class EntrySignalGenerator:
    ...
    def analyze(
        self, data: pd.DataFrame, timeframe: str = "1H"
    ...
    def _detect_triangle_breakout(self, data: pd.DataFrame, index: int) -> bool:
        ...
        triangle_indicators = [
            "triangle_breakout",
            "contracting_triangle_breakout",
            "expanding_triangle_breakout",
            "triangle_complete",
        ]
    ...
    def _create_wave3_entry_signal(
        self, data: pd.DataFrame, index: int, timeframe: str
    ...
    def _create_abc_entry_signal(
        ...
        )  # Corrective pattern signals are typically moderate
    ...
    def _calculate_wave1_height(self, data: pd.DataFrame, index: int) -> float:
        ...
        if wave1_start is None or wave1_end is None:
            ...
            recent_range = (
                data.iloc[index - look_back : index + 1]["high"].max()
                - data.iloc[index - look_back : index + 1]["low"].min()
            )
