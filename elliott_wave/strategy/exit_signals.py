...
from fxml3.strategy.entry_signals import SignalStrength, SignalType
...
class ExitSignalGenerator:
    ...
    def analyze(
        ...
        for position_id, position in self.active_positions.items():
            ...
            tp_signals = self._generate_take_profit_signals(
                wave_data, position, timeframe
            )

            # Generate stop loss signals
            sl_signals = self._generate_stop_loss_signals(
                wave_data, position, timeframe
            )

            # Generate pattern completion exit signals
            pc_signals = self._generate_pattern_completion_signals(
                wave_data, position, timeframe
            )

            # Generate trailing stop signals
            ts_signals = self._generate_trailing_stop_signals(
                wave_data, position, timeframe
            )

            # Generate time-based exit signals
            tb_signals = self._generate_time_based_signals(
                wave_data, position, timeframe
            )
    ...
    def _generate_take_profit_signals(
        self, data: pd.DataFrame, position: Dict[str, Any], timeframe: str
    ) -> Dict[pd.Timestamp, List[ExitSignal]]:
        """
                ...
                partial_levels.append(
                    {
                        ...
                        "price": partial_price,
                        "percentage": (
                            0.25 if level < 1.0 else 1.0
                        ),  # Exit 25% at partial levels, full at 100%
                    }
                )
        ...
        for i in range(len(data)):
            ...
            timestamp = (
                ...
                else pd.Timestamp(row["timestamp"])
            )
            ...
            if position_type == "long" and row["high"] >= take_profit:
                ...
                exit_signal = ExitSignal(
                    ...
                    additional_info={"position_id": position_id, "target_type": "main"},
                )
        ...
        for i in range(len(data)):
            ...
            if position_type == "long" and row["low"] <= stop_loss:
                ...
                exit_signal = ExitSignal(
                    ...
                    partial_exit=False,
                    partial_percentage=1.0,
                    additional_info={"position_id": position_id},
                )
