...
class FractalDegreeHandler:
    ...
    def analyze_timeframes(self, data_dict: Dict[str, pd.DataFrame]) -> Dict[str, Dict]:
    ...
    def _identify_nested_structures(self) -> Dict:
        ...
        for i in range(len(ordered_timeframes) - 1):
            ...
            higher_wave_cols = [
                col
                for col in higher_labeled.columns
                if col.startswith("impulse_") or col.startswith("corrective_")
            ]
            lower_wave_cols = [
                col
                for col in lower_labeled.columns
                if col.startswith("impulse_") or col.startswith("corrective_")
            ]
                ...
                for h_wave_num in higher_waves[h_col].unique():
                    ...
                    for l_col in lower_wave_cols:
                        ...
                        for l_wave_num in l_waves_in_range[l_col].unique():
                            l_wave = l_waves_in_range[
                                l_waves_in_range[l_col] == l_wave_num
                            ]
    ...
    def get_wave_annotations(
        self, timeframe: str, wave_col: str, use_degree_labels: bool = True
    ) -> Dict[int, str]:
    ...
    def label_nested_waves(
        ...
        if show_higher_degree:
            ...
            higher_relationships = [
                rel
                for rel, data in self.wave_relationships.items()
                if data["higher_degree"]["timeframe"] != timeframe
                and any(
                    wave
                    for wave_key, wave in data["lower_degree_waves"].items()
                    if timeframe in self.wave_patterns
                )
            ]
            ...
            for rel in higher_relationships:
                ...
                if higher_type == "impulse":
                    ...
                    higher_label = self.DEGREE_LABELS[higher_degree][
                        higher_number + 4
                    ]  # +4 to get to corrective waves
                ...
                mask = (result_df.index >= higher_start) & (
                    result_df.index <= higher_end
                )
                result_df.loc[mask, "higher_degree_wave"] = (
                    f"{higher_degree} {higher_label}"
                )
                ...
                if total_duration > 0:
                    result_df.loc[mask, "higher_degree_position"] = (
                        (result_df.index - higher_start).map(
        ...
        if show_lower_degree and self.wave_relationships:
            ...
            for rel in lower_relationships:
                ...
                for wave_key, wave_info in lower_waves.items():
                    lower_tf = (
                        wave_info["timeframe"] if "timeframe" in wave_info else "lower"
                    )
    ...
    def get_complete_wave_structure(self) -> Dict:
        ...
        for degree in ordered_degrees:
            timeframe = self._get_timeframe_from_degree(degree)
            if (
                timeframe in self.wave_patterns
                and self.wave_patterns[timeframe]["wave_points"]
            ):
        ...
        wave_cols = [
            col
            for col in highest_labeled.columns
            if col.startswith("impulse_") or col.startswith("corrective_")
        ]
    ...
    def _build_wave_structure(self, wave_key: str) -> Dict:
        ...
        for subwave_key, subwave_info in lower_waves.items():
            ...
            if subwave_key in self.wave_relationships:
                structure["subwaves"][subwave_key] = self._build_wave_structure(
                    subwave_key
                )
            else:
                structure["subwaves"][subwave_key] = {
                    "degree": self._get_degree_from_timeframe(
                        subwave_info.get("timeframe", "lower")
                    ),
