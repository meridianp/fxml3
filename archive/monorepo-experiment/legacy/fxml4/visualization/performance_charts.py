...
try:
    ...
    logger.warning("Matplotlib and/or seaborn not installed. Plotting functions will not work.")
    HAS_PLOTTING = False
...
def plot_equity_curve(
    ...
    if benchmark_df is not None:
        ...
        if isinstance(benchmark_df, pd.DataFrame):
            ...
            benchmark_values = benchmark_df[bench_col] * (equity_df['equity'].iloc[0] / benchmark_df[bench_col].iloc[0])
            ax.plot(benchmark_df.index, benchmark_values, label='Benchmark', linewidth=2, alpha=0.7)
...
def create_performance_dashboard(
    ...
    if save_path:
        for i, fig in enumerate(figures):
            if fig is not None:
                fig.savefig(f"{save_path}/performance_plot_{i+1}.png", dpi=300, bbox_inches='tight')
