...
class SquareRootModel(MarketImpactModel):
    ...
    def calculate_impact(
        ...
        if market_data is not None and "volume" in market_data.columns and len(market_data) >= self.adv_window:
...
class DecayingImpactModel(MarketImpactModel):
    ...
    def calculate_impact(
        ...
        if symbol in self.current_impact:
            ...
            time_diff = (timestamp - last_update).total_seconds() / 60.0  # Convert to minutes
