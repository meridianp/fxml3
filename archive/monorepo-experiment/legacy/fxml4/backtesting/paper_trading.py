...
from fxml4.backtesting.event import MarketEvent, SignalEvent, OrderEvent, FillEvent
...
class PaperTradingEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Set up data feed
        ib_config = config.get("ib_config", {
            "host": "127.0.0.1",
        ...
        # Working hours
        self.working_hours = config.get("working_hours", {
            "enabled": False,
    ...
    def start(self):
        ...
        if not hasattr(self.data_feed, 'app') or not self.data_feed.app.connected:
            if not self.connect():
                logger.error("Failed to connect to Interactive Brokers, cannot start paper trading engine")
                return
        ...
        self.data_thread = threading.Thread(target=self._process_market_data, daemon=True)
    ...
    def get_portfolio_status(self) -> Dict[str, Any]:
        ...
        for symbol, position in self.portfolio["positions"].items():
            ...
            if latest_tick and "bid" in latest_tick and "ask" in latest_tick:
                ...
                if position["direction"] == "LONG":
                    ...
                    position["unrealized_pnl"] = position["size"] * (mid_price - position["entry_price"])
                else:
                    position["current_price"] = mid_price
                    position["current_value"] = position["size"] * mid_price
                    position["unrealized_pnl"] = position["size"] * (position["entry_price"] - mid_price)
        
        # Calculate total equity
        total_position_value = sum([pos["current_value"] for pos in self.portfolio["positions"].values()])
        ...
        status = {
            ...
            "trades_today": sum(1 for trade in self.trade_history if 
                               (current_time - trade["timestamp"]).total_seconds() < 86400),
            "drawdown": float(self.drawdowns.iloc[-1]) if not self.drawdowns.empty else 0.0,
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        if len(returns) > 0:
            ...
            daily_return = ((self.equity_curve.iloc[-1] / self.equity_curve.iloc[0]) ** (252 / trading_days)) - 1
        else:
            daily_return = 0
        
        # Calculate metrics
        total_return = ((self.equity_curve.iloc[-1] / self.initial_capital) - 1) * 100 if len(self.equity_curve) > 0 else 0
        ...
        return {
            ...
            "current_equity": float(self.equity_curve.iloc[-1]) if not self.equity_curve.empty else self.initial_capital,
    ...
    def _fetch_initial_data(self):
        ...
        for symbol in self.symbols:
            try:
                ...
                if not df.empty:
                    logger.info(f"Fetched {len(df)} historical {self.signal_timeframe} bars for {symbol}")
                    
                    # Initialize timeframe converter with this data
                    if self.timeframe_converter:
                        self.timeframe_converter.update_data(symbol, df, self.signal_timeframe)
                else:
    ...
    def _is_trading_active(self) -> bool:
        ...
        # Parse working hours
        start_time = datetime.strptime(self.working_hours.get("start_time", "00:00"), "%H:%M").time()
        end_time = datetime.strptime(self.working_hours.get("end_time", "23:59"), "%H:%M").time()
        ...
        while self.is_running:
            try:
                ...
                for symbol in self.symbols:
                    ...
                    if (symbol in self.last_processed_time and 
                        self.signal_timeframe in self.last_processed_time[symbol] and
                        latest_candle_time > self.last_processed_time[symbol][self.signal_timeframe]):
                        
                        # Update last processed time
                        self.last_processed_time[symbol][self.signal_timeframe] = latest_candle_time
                        ...
                        logger.debug(f"Created market event for {symbol} at {latest_candle_time}")
        ...
        while self.is_running:
            try:
                ...
                if len(data) < 30:  # Need enough data for indicators
                    logger.debug(f"Not enough data for {symbol} ({len(data)} bars), skipping signal generation")
                    self.market_event_queue.task_done()
                    continue
                ...
                if (symbol in self.last_signal_time and 
                    (current_time - self.last_signal_time[symbol]).total_seconds() < self.signal_cooldown * 60):
                    logger.debug(f"Signal cooldown active for {symbol}, skipping signal generation")
                    self.market_event_queue.task_done()
                    continue
                ...
                for generator in self.signal_generators:
                    try:
                        ...
                        if hasattr(generator, 'supports_symbol') and not generator.supports_symbol(symbol):
                            continue
                        ...
                        if signal and signal.signal_type in ["LONG", "SHORT", "EXIT"]:
                            ...
                            logger.info(f"Generated {signal.signal_type} signal for {symbol} from {generator_name}")
                    except Exception as e:
                        logger.error(f"Error generating signal with {generator.__class__.__name__}: {e}")
                ...
                if signals:
                    ...
                    if len(signals) > 1:
                        ...
                        if combined_signal:
                            ...
                            logger.info(f"Combined signal {combined_signal.signal_type} for {symbol}")
                    else:
    ...
    def _combine_signals(self, signals: List[SignalEvent], weights: Dict[str, float]) -> Optional[SignalEvent]:
    ...
    def _process_orders(self):
        ...
        while self.is_running:
            try:
                ...
                latest_tick = self.data_feed.get_latest_tick(symbol)
                if not latest_tick or "bid" not in latest_tick or "ask" not in latest_tick:
                    logger.warning(f"No market data available for {symbol}, skipping order generation")
                ...
                position_direction = self.portfolio["positions"].get(symbol, {}).get("direction", None)
                ...
                if not order_type:
                    logger.debug(f"No order generated for {symbol} signal {signal_type}, position: {position_direction}")
                    self.signal_event_queue.task_done()
                    continue
                
                # Check risk constraints
                max_position_size = self._calculate_position_size(symbol, order_type, mid_price)
                
                if max_position_size <= 0:
                    logger.warning(f"Risk constraints prevent order for {symbol} ({order_type})")
                    self.signal_event_queue.task_done()
                    continue
                ...
                logger.info(f"Created {order_type} order for {symbol}: {max_position_size} units at {mid_price:.5f}")
    ...
    def _calculate_position_size(self, symbol: str, order_type: str, price: float) -> float:
        ...
        if current_positions >= self.max_positions and symbol not in self.portfolio["positions"]:
            logger.warning(f"Maximum positions limit reached ({self.max_positions}), no new positions allowed")
            return 0.0
    ...
    def _process_fills(self):
        ...
        while self.is_running:
            try:
                ...
                latest_tick = self.data_feed.get_latest_tick(symbol)
                if not latest_tick or "bid" not in latest_tick or "ask" not in latest_tick:
                    logger.warning(f"No market data available for {symbol}, using original price")
                else:
                ...
                # Apply slippage
                fill_price = self.slippage_model.apply_slippage(price, order_type, quantity)
    ...
    def _process_fill_event(self, fill: FillEvent):
        ...
        if direction in ["BUY", "COVER"]:
            ...
            if cost > self.portfolio["cash"]:
                logger.warning(f"Insufficient cash for {direction} {symbol}: {cost} > {self.portfolio['cash']}")
                return
            ...
            if direction == "COVER" and symbol in self.portfolio["positions"]:
                ...
                if position["direction"] == "SHORT":
                    ...
                    if cover_size < position_size:
                        ...
                        logger.info(f"Partially covered {symbol} short: {cover_size} units at {fill_price:.5f}, PnL: {pnl:.2f}")
                    else:
                        # Full cover
                        del self.portfolio["positions"][symbol]
                        logger.info(f"Fully covered {symbol} short: {cover_size} units at {fill_price:.5f}, PnL: {pnl:.2f}")
                    
                    # Update cash
                    self.portfolio["cash"] += pnl
                    
                    # Record trade
                    self._record_trade(symbol, "COVER", entry_price, fill_price, cover_size, pnl)
                    ...
                    if cover_size < quantity and direction == "COVER":
                        ...
                        logger.info(f"Opened new {symbol} long: {long_size} units at {fill_price:.5f}")
                else:
                    logger.warning(f"Attempted to cover {symbol} but position is not short")
            else:
                # New long position
                self.portfolio["positions"][symbol] = {
                    "direction": "LONG",
                ...
                logger.info(f"Opened new {symbol} long: {quantity} units at {fill_price:.5f}")
            ...
            if symbol in self.portfolio["positions"]:
                ...
                if position["direction"] == "LONG":
                    ...
                    if sell_size < position_size:
                        ...
                        logger.info(f"Partially closed {symbol} long: {sell_size} units at {fill_price:.5f}, PnL: {pnl:.2f}")
                    else:
                        # Full sell
                        del self.portfolio["positions"][symbol]
                        logger.info(f"Fully closed {symbol} long: {sell_size} units at {fill_price:.5f}, PnL: {pnl:.2f}")
                    
                    # Update cash (add sale proceeds and profit)
                    self.portfolio["cash"] += (sell_size * fill_price)
                    
                    # Record trade
                    self._record_trade(symbol, "SELL", entry_price, fill_price, sell_size, pnl)
                    ...
                    if sell_size < quantity:
                        ...
                        logger.info(f"Opened new {symbol} short: {short_size} units at {fill_price:.5f}")
                else:
                    # Position is already short, increase it
                    position["size"] += quantity
                    # For adding to position, use weighted average price
                    position["entry_price"] = ((position["size"] - quantity) * position["entry_price"] + 
                                               quantity * fill_price) / position["size"]
                    logger.info(f"Increased {symbol} short position to {position['size']} units, avg price: {position['entry_price']:.5f}")
            else:
                # New short position
                self.portfolio["positions"][symbol] = {
                    "direction": "SHORT",
                ...
                logger.info(f"Opened new {symbol} short: {quantity} units at {fill_price:.5f}")
    ...
    def _record_trade(self, symbol: str, direction: str, entry_price: float, exit_price: float, 
    ...
    def _store_fill(self, fill: FillEvent):
        ...
        try:
            ...
            self.db_client.execute_query(query, values)
            logger.debug(f"Stored fill event in database: {fill.symbol} {fill.direction}")
        except Exception as e:
    ...
    def _store_trade(self, trade: Dict[str, Any]):
        ...
        try:
            ...
            self.db_client.execute_query(query, values)
            logger.debug(f"Stored trade in database: {trade['symbol']} {trade['pnl']:.2f}")
        except Exception as e:
