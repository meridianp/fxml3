...
class KellyPositionSizer(PositionSizer):
    ...
    def calculate_position_size(
        ...
        win_rate, win_loss_ratio = self._calculate_statistics(
    ...
    def _calculate_statistics(
        ...
        closed_positions = closed_positions[-self.lookback_trades:] if closed_positions else []
        ...
        if winning_trades:
            avg_win = sum(p.get("realized_pnl", 0) for p in winning_trades) / len(winning_trades)
        else:
            avg_win = 0
        
        losing_trades = [p for p in closed_positions if p.get("realized_pnl", 0) <= 0]
        if losing_trades:
            avg_loss = abs(sum(p.get("realized_pnl", 0) for p in losing_trades) / len(losing_trades))
        else:
...
class OptimalFPositionSizer(PositionSizer):
    ...
    def _calculate_optimal_f(self, portfolio: Any, symbol: Optional[str] = None) -> float:
        """Calculate Optimal-F from portfolio history.
    ...
    def create_stop_loss(
        ...
        if "stop_loss" in signal.signal_data:
            ...
            if stop_type == StopLossType.FIXED.value:
                ...
                stop_price = entry_price - fixed_amount if signal.signal_data.get("side") == "buy" else entry_price + fixed_amount
            
            elif stop_type == StopLossType.PERCENTAGE.value:
                percentage = signal.signal_data.get("stop_percentage", self.default_percentage)
                stop_price = entry_price * (1 - percentage) if signal.signal_data.get("side") == "buy" else entry_price * (1 + percentage)
            
            elif stop_type == StopLossType.VOLATILITY.value and market_data is not None:
                atr_multiple = signal.signal_data.get("atr_multiple", self.default_atr_multiple)
                atr = self._calculate_atr(market_data)
                stop_price = entry_price - (atr * atr_multiple) if signal.signal_data.get("side") == "buy" else entry_price + (atr * atr_multiple)
            
            elif stop_type == StopLossType.TRAILING.value:
                # For trailing stops, set initial stop price
                trailing_percentage = signal.signal_data.get("trailing_percentage", self.default_trailing_percentage)
                stop_price = entry_price * (1 - trailing_percentage) if signal.signal_data.get("side") == "buy" else entry_price * (1 + trailing_percentage)
                ...
                if signal.signal_data.get("side") == "buy":
                    recent_high = market_data["high"].iloc[-20:].max() if len(market_data) >= 20 else entry_price
                    stop_price = recent_high - (atr * atr_multiple)
                else:
                    recent_low = market_data["low"].iloc[-20:].min() if len(market_data) >= 20 else entry_price
                ...
                time_limit = signal.signal_data.get("time_limit", self.default_time_limit)
                if time_limit:
        ...
        order = OrderEvent(
            ...
            additional_params={
                ...
                "trailing_percentage": signal.signal_data.get("trailing_percentage", self.default_trailing_percentage) if stop_type == StopLossType.TRAILING.value else None,
            },
        )
        ...
        self.stop_orders[position_id] = {
            ...
            "trailing": stop_type == StopLossType.TRAILING.value,
            "trailing_percentage": signal.signal_data.get("trailing_percentage", self.default_trailing_percentage) if stop_type == StopLossType.TRAILING.value else None,
            "highest_price": entry_price if signal.signal_data.get("side") == "buy" else None,
            "lowest_price": entry_price if signal.signal_data.get("side") == "sell" else None,
        }
    ...
    def update_stops(
        ...
        for position_id, stop_info in self.stop_orders.items():
            ...
            if stop_info.get("type") == StopLossType.TIME.value:
                ...
                if entry_time and time_limit and current_time - entry_time >= time_limit:
            ...
            if stop_info.get("trailing"):
                ...
                trailing_percentage = stop_info.get("trailing_percentage", self.default_trailing_percentage)
                ...
                if side == "buy":
                    ...
                    if current_price > highest_price:
                        ...
                        if new_stop > stop_info.get("stop_price", 0):
                            ...
                            order = OrderEvent(
                                timestamp=current_time,
                                order_id=stop_info.get("order_id", f"trailing-{uuid.uuid4()}"),
                                symbol=symbol,
                                order_type="stop",
                                side="sell",
                                quantity=stop_info.get("quantity", 0),
                                ...
                                parent_order_id=position_id,
                                signal_id=None,
                                additional_params={"stop_type": StopLossType.TRAILING.value},
                            )
                            updated_orders.append(order)
...
class DrawdownControl:
    ...
    def check_drawdown(
        ...
        if self.portfolio_peak > 0:
            portfolio_drawdown = (self.portfolio_peak - portfolio_value) / self.portfolio_peak
        
        # Check portfolio-level cooling off
        portfolio_cooling_off = False
        if self.portfolio_cooling_off_until and current_time < self.portfolio_cooling_off_until:
        ...
        if portfolio_drawdown > self.max_drawdown_pct and not portfolio_cooling_off:
            self.portfolio_cooling_off_until = current_time + timedelta(days=self.cooling_off_days)
            portfolio_cooling_off = True
        ...
        for symbol, value in symbol_values.items():
            ...
            if self.symbol_peaks[symbol] > 0:
                symbol_drawdown = (self.symbol_peaks[symbol] - value) / self.symbol_peaks[symbol]
            
            # Check symbol cooling off
            symbol_cooling_off = False
            if symbol in self.cooling_off_until and current_time < self.cooling_off_until[symbol]:
                symbol_cooling_off = True
            
            # Set new cooling off period if max drawdown exceeded
            if symbol_drawdown > self.per_symbol_drawdown_pct and not symbol_cooling_off:
                self.cooling_off_until[symbol] = current_time + timedelta(days=self.cooling_off_days)
                symbol_cooling_off = True
            ...
            symbol_status[symbol] = {
                ...
                "reduction_factor": self.reduction_factor if symbol_cooling_off else 1.0,
            }
        
        return {
            "portfolio_drawdown": portfolio_drawdown,
            "portfolio_cooling_off": portfolio_cooling_off,
            "portfolio_reduction_factor": self.reduction_factor if portfolio_cooling_off else 1.0,
...
class RiskManager:
    ...
    def process_signal(
        ...
        if len(portfolio.positions) >= self.max_positions and signal.signal_type == "entry":
            logger.info("Skipping signal: maximum positions reached (%d)", self.max_positions)
            return None, None
        ...
        if self.avoid_high_impact_news and self.news_filter and signal.signal_type == "entry":
            ...
            if is_news_time:
                ...
                if reason == "abnormal_spread":
                    logger.info(f"Skipping signal: abnormal spread detected [{event_names}]")
                else:
                    logger.info(f"Skipping signal: high impact news event(s) [{event_names}]")
                return None, None
        ...
        if signal.signal_type == "entry":
            ...
            if symbol in portfolio.positions:
                logger.info("Skipping entry signal: position already exists for %s", symbol)
                return None, None
            
            # Check correlation limits
            if not self._check_correlation_limits(symbol, portfolio):
                logger.info("Skipping entry signal: correlation limits reached for %s", symbol)
                return None, None
            ...
            risk_amount = self._calculate_risk_amount(signal, portfolio, current_price, quantity)
            if risk_amount / portfolio.equity > self.risk_per_trade_pct:
                logger.info("Reducing position size: risk per trade limit exceeded")
                quantity = quantity * (self.risk_per_trade_pct * portfolio.equity / risk_amount)
            
            if self.daily_risk_used + risk_amount / portfolio.equity > self.max_risk_per_day_pct:
    ...
    def update_risk_controls(
        ...
        self.drawdown_control.check_drawdown(portfolio.equity, symbol_values, current_time)
    ...
    def _check_news_events(self, portfolio: Any, current_time: datetime) -> List[OrderEvent]:
        """Check for news events and generate exit orders if needed.
        ...
        for symbol, position in portfolio.positions.items():
            ...
            if is_news_time:
                ...
                if reason == "abnormal_spread":
                    logger.info(f"Exiting position for {symbol} due to abnormal spread: {event_names}")
                elif reason == "economic_release":
                    logger.info(f"Exiting position for {symbol} due to economic release: {event_names}")
                else:
                    logger.info(f"Exiting position for {symbol} due to news event(s): {event_names}")
    ...
    def _check_leverage_limits(
        ...
        current_leverage = total_position_value / portfolio.equity if portfolio.equity > 0 else 0
        ...
        new_leverage = (total_position_value + new_position_value) / portfolio.equity if portfolio.equity > 0 else 0
...
class PortfolioRiskMetrics:
    ...
    def _should_calculate(self, current_time: datetime) -> bool:
        ...
        if self.calculation_frequency == "daily":
            ...
            return (current_time.year, current_time.month) > (self.last_calculation.year, self.last_calculation.month)
    ...
    def _calculate_sharpe_ratio(
        ...
        sharpe = (returns.mean() - self.risk_free_rate / annualization_factor) / returns.std()
    ...
    def _calculate_sortino_ratio(
        ...
        sortino = (returns.mean() - self.risk_free_rate / annualization_factor) / downside_returns.std()
    ...
    def _calculate_benchmark_metrics(
        ...
        information_ratio = (portfolio_returns.mean() - benchmark_returns.mean()) / tracking_error if tracking_error > 0 else 0
        ...
        up_capture = (portfolio_returns[up_market].mean() / benchmark_returns[up_market].mean()) if up_market.any() and benchmark_returns[up_market].mean() != 0 else 0
        down_capture = (portfolio_returns[down_market].mean() / benchmark_returns[down_market].mean()) if down_market.any() and benchmark_returns[down_market].mean() != 0 else 0
    ...
    def _calculate_risk_exposure(self, portfolio: Any) -> Dict[str, Any]:
        ...
        long_value = sum(pos.get("current_value", 0) for pos in positions.values() if pos.get("side") == "buy")
        short_value = sum(pos.get("current_value", 0) for pos in positions.values() if pos.get("side") == "sell")
        ...
        if not positions:
            ...
            weights = [(pos.get("current_value", 0) / total_value) ** 2 for pos in positions.values()]
