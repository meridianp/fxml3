...
class VolatilityRegimeDetector:
    ...
    def update_volatility(self, symbol: str, returns: pd.Series) -> None:
        ...
        if len(self.volatility_history[symbol]) > self.lookback_periods:
            self.volatility_history[symbol] = self.volatility_history[symbol][-self.lookback_periods:]
    
    def get_regime(self, symbol: str, current_volatility: Optional[float] = None) -> VolatilityRegime:
        ...
        """
        if symbol not in self.volatility_history or len(self.volatility_history[symbol]) < 20:
...
class PerformanceTracker:
    ...
    def _update_metrics(self) -> None:
        ...
        self.profit_factors["recent"] = gross_profit / gross_loss if gross_loss > 0 else float('inf')
        ...
        if returns:
            ...
            self.sharpe_ratios["recent"] = mean_return / std_return * np.sqrt(252) if std_return > 0 else 0
...
class EnhancedKellyPositionSizer(PositionSizer):
    ...
    def calculate_position_size(
        ...
        if "metadata" in signal.signal_data:
            ml_confidence = signal.signal_data["metadata"].get("raw_probability", ml_confidence)
        ...
        if avg_loss > 0:
            ...
            kelly_pct = (confidence_adjusted_win_rate * win_loss_ratio - (1 - confidence_adjusted_win_rate)) / win_loss_ratio
        else:
            kelly_pct = 0
...
class DynamicPositionSizer(PositionSizer):
    ...
    def calculate_position_size(
        ...
        base_size = self.base_sizer.calculate_position_size(signal, portfolio, current_price)
        ...
        drawdown_multiplier = 1.0 - (current_drawdown * 0.5)  # Reduce by up to 50% in drawdown
        ...
        # Apply constraints
        total_multiplier = max(self.min_size_multiplier, min(self.max_size_multiplier, total_multiplier))
        ...
        if self.use_correlation and self.correlation_matrix is not None:
            ...
            if current_positions:
                ...
                if symbol in self.correlation_matrix.columns:
                    correlations = self.correlation_matrix.loc[symbol, position_symbols].values
                    ...
                    correlation_adjustment = 1 + np.sum(correlations * position_weights) * 0.5
