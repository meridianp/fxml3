...
class ElliottWaveChartGenerator:
    ...
    def _prepare_additional_plots(self, data: pd.DataFrame, indicators: List[str]) -> List:
    ...
    def _add_wave_annotations(self, ax, data: pd.DataFrame, wave_patterns: List[Dict]):
        ...
        for pattern in wave_patterns:
            ...
            for i, wave in enumerate(waves):
                ...
                # Draw wave line
                color = self.colors['impulse'] if pattern_type == 'impulse' else self.colors['corrective']
                ax.plot([x1, x2], [y1, y2], color=color, alpha=0.7)
                
                # Add wave label
                wave_label = str(i+1) if pattern_type == 'impulse' else chr(65+i)  # 1-5 or A-C
                mid_x = (x1 + x2) / 2
                mid_y = (y1 + y2) / 2
                ...
                ax.text(mid_x, mid_y, wave_label, 
                       bbox=bbox_props,
                       fontsize=10,
                       fontweight='bold',
                       ha='center',
        ...
        if wave_patterns:
            ...
            ax.text(0.02, 0.98, pattern_text,
    ...
    def _add_fibonacci_levels(self, ax, fib_levels: Dict, data: pd.DataFrame):
        ...
        for level_name, level_price in fib_levels.items():
            if y_min <= level_price <= y_max:
                # Draw level
                ax.axhline(y=level_price, 
                          color=self.colors['fibonacci'],
                          linestyle='--',
                          alpha=0.6)
                
                # Add label
                ax.text(x_max * 0.98, level_price, f' {level_name}', 
                       ha='right',
                       va='center',
    ...
    def _add_support_resistance(self, ax, data: pd.DataFrame):
        ...
        for resistance in resistance_levels:
            ax.axhline(y=resistance,
                      color=self.colors['resistance'],
                      linestyle=':',
                      alpha=0.5)
            ax.text(x_max * 0.02, resistance, 'R', 
                   color=self.colors['resistance'],
                   fontweight='bold',
                   va='center')
        
        for support in support_levels:
            ax.axhline(y=support,
                      color=self.colors['support'],
                      linestyle=':',
                      alpha=0.5)
            ax.text(x_max * 0.02, support, 'S',
                   color=self.colors['support'],
                   fontweight='bold',
                   va='center')
...
def create_multi_timeframe_chart(
    ...
    generator.generate_elliott_wave_chart(
        data_4h.tail(100),
        wave_patterns_4h,
        title=f"{symbol} 4H Elliott Wave Analysis"
    )
