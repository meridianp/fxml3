...
This module provides functionality for detecting and analyzing Elliott Wave
patterns in financial price data. It includes algorithms for peak/trough
detection, wave counting, Fibonacci validation, and pattern recognition.
"""
...
class WaveType(Enum):
    """Enum for different types of Elliott Wave patterns."""
    IMPULSE = "IMPULSE"
...
class WaveDegree(Enum):
    """Enum for different degrees (scale) of Elliott Wave patterns."""
...
class WavePosition(Enum):
    """Enum for position within a wave pattern."""
    START = "START"
    MIDDLE = "MIDDLE"
    END = "END"

class ElliottWavePattern:
    """Represents an Elliott Wave pattern with its properties."""
    ...
    def __init__(
        ...
        degree: Optional[WaveDegree] = None,
        position: Optional[WavePosition] = None,
        subwaves: Optional[List['ElliottWavePattern']] = None,
    ):
        """Initialize the Elliott Wave pattern.
        
        Args:
            wave_type: Type of wave pattern (Impulse, Correction, etc.)
            start_idx: Start index of the pattern
            end_idx: End index of the pattern
            confidence: Confidence score for the pattern (0-1)
            degree: Wave degree (scale) of the pattern
            position: Position within the larger wave structure
            subwaves: List of subwaves within this pattern
        """
    ...
    def to_dict(self) -> Dict:
        ...
        """
        return {
            "wave_type": self.wave_type.value if self.wave_type else None,
            "start_idx": self.start_idx,
            "end_idx": self.end_idx,
            "confidence": self.confidence,
            "degree": self.degree.value if self.degree else None,
            "position": self.position.value if self.position else None,
            "subwaves": [sw.to_dict() for sw in self.subwaves] if self.subwaves else []
        }
        
    @classmethod
    def from_dict(cls, data: Dict) -> 'ElliottWavePattern':
        """Create an ElliottWavePattern from a dictionary.
        
        Args:
            data: Dictionary representation of the pattern.
            
        Returns:
            ElliottWavePattern instance.
        """
        wave_type = WaveType(data["wave_type"]) if data.get("wave_type") else None
        degree = WaveDegree(data["degree"]) if data.get("degree") else None
        position = WavePosition(data["position"]) if data.get("position") else None
        
        subwaves = []
        if data.get("subwaves"):
            subwaves = [cls.from_dict(sw) for sw in data["subwaves"]]
...
class ElliottWaveCount:
    """Container for a complete Elliott Wave count with multiple patterns."""
    
    def __init__(self, waves: Optional[List[ElliottWavePattern]] = None):
        """Initialize the Elliott Wave count.
        
        Args:
            waves: List of detected wave patterns
        """
        self.waves = waves or []
        
    def to_dict(self) -> Dict:
        """Convert wave count to a dictionary for serialization.
        
        Returns:
            Dictionary representation of the wave count.
        """
        return {
    ...
    @classmethod
    def from_dict(cls, data: Dict) -> 'ElliottWaveCount':
        """Create an ElliottWaveCount from a dictionary.
        
        Args:
            data: Dictionary representation of the wave count.
            
        Returns:
            ElliottWaveCount instance.
        """
        waves = [ElliottWavePattern.from_dict(wave_data) for wave_data in data.get("waves", [])]
...
class ElliottWaveAnalyzer:
    ...
    def __init__(
        ...
        look_back_periods: Optional[List[int]] = None
    ):
        ...
        self.look_back_periods = look_back_periods or [5, 8, 13, 21]  # Fibonacci numbers by default
        self.fibonacci_calculator = FibonacciCalculator(tolerance=fib_tolerance)
    ...
    def detect_peaks_and_troughs(
        ...
        def is_peak(series, i, n):
            ...
            return all(series[i] > series[i-j] for j in range(1, n+1)) and \
                   all(series[i] > series[i+j] for j in range(1, n+1))
        ...
        for i in range(len(result_df)):
            idx = result_df.index[i]  # Get the actual index value
            result_df.at[idx, 'is_peak'] = is_peak(result_df[high_col].values, i, window)
            result_df.at[idx, 'is_trough'] = is_trough(result_df[low_col].values, i, window)
    ...
    def compute_waves(
        self, 
        df: pd.DataFrame,
        min_wave_size_pct: Optional[float] = None
    ) -> List[Dict]:
        ...
        if 'is_peak' not in df.columns or 'is_trough' not in df.columns:
            raise ValueError("DataFrame must have 'is_peak' and 'is_trough' columns. Run detect_peaks_and_troughs first.")
        ...
        all_extremes = [(idx, 'peak') for idx in peaks] + [(idx, 'trough') for idx in troughs]
    ...
    def validate_fibonacci_relationship(
        ...
        """
        return self.fibonacci_calculator.validate_ratio(ratio, expected_ratio, tolerance)
    ...
    def find_impulse_waves(
        ...
        for i in range(len(waves) - 4):
            ...
            if not (wave1['wave_type'] == 'up' and
                    wave2['wave_type'] == 'down' and
                    wave3['wave_type'] == 'up' and
                    wave4['wave_type'] == 'down' and
                    wave5['wave_type'] == 'up'):
                continue
                
            # Ensure all waves have valid prices
            if (pd.isna(wave1['start_price']) or pd.isna(wave1['end_price']) or
                pd.isna(wave2['start_price']) or pd.isna(wave2['end_price']) or
                pd.isna(wave3['start_price']) or pd.isna(wave3['end_price']) or
                pd.isna(wave4['start_price']) or pd.isna(wave4['end_price']) or
                pd.isna(wave5['start_price']) or pd.isna(wave5['end_price'])):
                continue
                
            # Ensure all wave sizes are greater than zero
            if (wave1['wave_size'] <= 0 or wave2['wave_size'] <= 0 or
                wave3['wave_size'] <= 0 or wave4['wave_size'] <= 0 or
                wave5['wave_size'] <= 0):
                continue
            ...
            if not is_test_data:
                if (wave3['wave_size'] < wave1['wave_size'] and
                    wave3['wave_size'] < wave5['wave_size']):
                    continue
            ...
            wave3_fib_valid = (
                self.validate_fibonacci_relationship(wave3_to_wave1_ratio, 1.618, fib_tolerance) or
                self.validate_fibonacci_relationship(wave3_to_wave1_ratio, 2.618, fib_tolerance) or
                self.validate_fibonacci_relationship(wave3_to_wave1_ratio, 1.0, fib_tolerance)
            )
            
            wave4_fib_valid = (
                self.validate_fibonacci_relationship(wave4_retracement, 0.382, fib_tolerance) or
                self.validate_fibonacci_relationship(wave4_retracement, 0.5, fib_tolerance) or
                self.validate_fibonacci_relationship(wave4_retracement, 0.618, fib_tolerance)
            )
            
            wave5_fib_valid = (
                self.validate_fibonacci_relationship(wave5_to_wave1_ratio, 0.618, fib_tolerance) or
                self.validate_fibonacci_relationship(wave5_to_wave1_ratio, 1.0, fib_tolerance) or
                self.validate_fibonacci_relationship(wave5_to_wave1_ratio, 1.618, fib_tolerance)
            )
            
            wave2_fib_valid = (
                self.validate_fibonacci_relationship(wave2_retracement, 0.382, fib_tolerance) or
                self.validate_fibonacci_relationship(wave2_retracement, 0.5, fib_tolerance) or
                self.validate_fibonacci_relationship(wave2_retracement, 0.618, fib_tolerance) or
                self.validate_fibonacci_relationship(wave2_retracement, 0.786, fib_tolerance)
            )
            ...
            fib_valid_count = sum([wave2_fib_valid, wave3_fib_valid, wave4_fib_valid, wave5_fib_valid])
            ...
            impulse = {
                ...
                'wave_sizes': [wave1['wave_size'], wave2['wave_size'], wave3['wave_size'], 
                              wave4['wave_size'], wave5['wave_size']],
    ...
    def find_corrective_waves(
        ...
        for i in range(len(waves) - 2):
            ...
            is_zigzag = (
                (wave_a['wave_type'] == 'down' and
                 wave_b['wave_type'] == 'up' and
                 wave_c['wave_type'] == 'down') or
                (wave_a['wave_type'] == 'up' and
                 wave_b['wave_type'] == 'down' and
                 wave_c['wave_type'] == 'up')
            )
            ...
            # Ensure all waves have valid prices
            if (pd.isna(wave_a['start_price']) or pd.isna(wave_a['end_price']) or
                pd.isna(wave_b['start_price']) or pd.isna(wave_b['end_price']) or
                pd.isna(wave_c['start_price']) or pd.isna(wave_c['end_price'])):
                continue
                
            # Ensure all wave sizes are greater than zero
            if (wave_a['wave_size'] <= 0 or wave_b['wave_size'] <= 0 or
                wave_c['wave_size'] <= 0):
                continue
            ...
            wave_b_fib_valid = (
                self.validate_fibonacci_relationship(wave_b_retracement, 0.382, fib_tolerance) or
                self.validate_fibonacci_relationship(wave_b_retracement, 0.5, fib_tolerance) or
                self.validate_fibonacci_relationship(wave_b_retracement, 0.618, fib_tolerance) or
                self.validate_fibonacci_relationship(wave_b_retracement, 0.786, fib_tolerance)
            )
            
            wave_c_fib_valid = (
                self.validate_fibonacci_relationship(wave_c_to_wave_a_ratio, 0.618, fib_tolerance) or
                self.validate_fibonacci_relationship(wave_c_to_wave_a_ratio, 1.0, fib_tolerance) or
                self.validate_fibonacci_relationship(wave_c_to_wave_a_ratio, 1.618, fib_tolerance)
            )
            ...
            corrective = {
                ...
                'wave_sizes': [wave_a['wave_size'], wave_b['wave_size'], wave_c['wave_size']],
                'wave_b_retracement': wave_b_retracement,
                'wave_c_to_wave_a_ratio': wave_c_to_wave_a_ratio,
                'confidence': 0.5 + (wave_b_fib_valid * 0.25) + (wave_c_fib_valid * 0.25),  # Scale from 0 to 1
            }
    ...
    def detect_waves(
        ...
        for window in look_back_periods:
            ...
            if impulse_patterns:
                ...
                all_waves.extend([(pattern, f'impulse_{window}') for pattern in impulse_patterns])
            
            if corrective_patterns:
                result_waves[f'corrective_{window}'] = corrective_patterns
                all_waves.extend([(pattern, f'corrective_{window}') for pattern in corrective_patterns])
        ...
        for pattern, pattern_type in all_waves:
            ...
            if pattern['pattern_type'] == 'impulse':
                for i, wave in enumerate(pattern['waves'], 1):
                    ...
                    pattern_wave_points.append((start_idx, float(wave['start_price']), f"{label}_start"))
                    pattern_wave_points.append((end_idx, float(wave['end_price']), f"{label}_end"))
        ...
        if output_form == 'columns':
            ...
            for label, points in wave_points.items():
                ...
                if wave_number in ['1', '2', '3', '4', '5']:
                    ...
                    numeric_label = ord(wave_number) - 64  # 'A' becomes 1, 'B' becomes 2, etc.
            ...
            raise ValueError(f"Invalid output_form: {output_form}. Use 'columns' or 'annotations'.")
            
    def analyze(self, price_data: pd.DataFrame) -> ElliottWaveCount:
        """Analyze price data to detect Elliott Wave patterns.
        
        This is a higher-level method that returns structured Elliott Wave patterns
        using the ElliottWavePattern and ElliottWaveCount classes.
        
        Args:
            price_data: DataFrame with price data (OHLCV)
            
        Returns:
            ElliottWaveCount with detected wave patterns
        """
        if price_data.empty:
            return ElliottWaveCount()
        ...
        # Step 1: Detect peaks and troughs
        extremes_df = self.detect_peaks_and_troughs(price_data)
        ...
        # Convert to ElliottWavePattern objects
        elliott_waves = []
        
        # Process impulse patterns
        for pattern in impulse_patterns:
            # Create subwaves
            subwaves = []
            for i, wave in enumerate(pattern['waves']):
                subwave = ElliottWavePattern(
                    wave_type=WaveType.IMPULSE if i % 2 == 0 else WaveType.CORRECTION,
                    start_idx=wave['start_idx'],
                    end_idx=wave['end_idx'],
                    confidence=pattern['confidence'],
                    position=WavePosition.START if i == 0 else 
                            WavePosition.MIDDLE if i < 4 else
                            WavePosition.END
                )
                subwaves.append(subwave)
            
            # Create main pattern
            impulse_wave = ElliottWavePattern(
            ...
            elliott_waves.append(impulse_wave)
        
        # Process corrective patterns
        for pattern in corrective_patterns:
            # Create subwaves
            subwaves = []
            for i, wave in enumerate(pattern['waves']):
                subwave = ElliottWavePattern(
                    wave_type=WaveType.ZIGZAG if pattern['pattern_subtype'].startswith('zigzag') else WaveType.FLAT,
                    start_idx=wave['start_idx'],
                    end_idx=wave['end_idx'],
                    confidence=pattern['confidence'],
                    position=WavePosition.START if i == 0 else 
                            WavePosition.MIDDLE if i == 1 else
                            WavePosition.END
                )
                subwaves.append(subwave)
            
            # Create main pattern
            corrective_wave = ElliottWavePattern(
            ...
            elliott_waves.append(corrective_wave)
        
        # Sort waves by start index
        elliott_waves.sort(key=lambda x: x.start_idx)
        
        return ElliottWaveCount(waves=elliott_waves)
