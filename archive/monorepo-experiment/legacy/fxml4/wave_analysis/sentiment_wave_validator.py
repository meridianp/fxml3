...
from fxml4.wave_analysis.elliott_wave import (
    ElliottWaveAnalyzer, ElliottWavePattern, ElliottWaveCount, WaveType
)
...
class SentimentWaveValidator:
    ...
    def __init__(
        ...
        self.expected_sentiment = {
            ...
            (WaveType.IMPULSE, "END"): 0.2,  # Weakly bullish at end of impulse (exhaustion)
            
            (WaveType.CORRECTION, "START"): -0.3,  # Moderately bearish at start of correction
            ...
            (WaveType.DIAGONAL, "START"): 0.3,  # Moderately bullish at start of diagonal
            (WaveType.DIAGONAL, "MIDDLE"): 0.1,  # Slightly bullish during diagonal
            (WaveType.DIAGONAL, "END"): -0.2,  # Turning bearish at end of diagonal (often reversal)
            
            (WaveType.TRIANGLE, "START"): 0.0,  # Neutral at start of triangle (consolidation)
            (WaveType.TRIANGLE, "MIDDLE"): 0.0,  # Neutral during triangle
            (WaveType.TRIANGLE, "END"): 0.3,  # Turning bullish at end of triangle (breakout)
        }
    ...
    def validate_wave_pattern(
        ...
        try:
            ...
            sentiment_confidence = max(0, 1.0 - (sentiment_diff / self.sentiment_tolerance))
            ...
            if self.rag:
                rag_confidence = self._get_rag_validation(pattern, price_data, sentiment_score)
    ...
    def analyze_with_sentiment(
        ...
        try:
            ...
            if valid_patterns:
                results["combined_score"] = sum(p["confidence"] for p in valid_patterns) / len(valid_patterns)
