...
class DynamicExitStrategy:
    ...
    def initialize_position(
        ...
        if confidence < 0.7:
            ...
            if position_type == 'long':
                initial_stop = entry_price - self.config.initial_stop_atr_multiplier * atr * 0.8
            else:
                initial_stop = entry_price + self.config.initial_stop_atr_multiplier * atr * 0.8
    ...
    def update_position(
        ...
        # 6. Signal Reversal
        if current_signal is not None and self._check_signal_exit(pos, current_signal, signal_confidence):
            return True, current_price, ExitReason.SIGNAL_REVERSAL, pos['remaining_size']
            
        # 7. Volatility Exit
        if market_volatility and self._check_volatility_exit(pos, market_volatility, current_atr):
            return True, current_price, ExitReason.VOLATILITY_EXIT, pos['remaining_size']
    ...
    def _check_partial_profits(
        ...
        for i, (tp_level, portion) in enumerate(zip(pos['take_profit_levels'], pos['tp_portions'])):
    ...
    def _update_trailing_stop(self, pos: Dict, current_price: float, current_atr: float):
