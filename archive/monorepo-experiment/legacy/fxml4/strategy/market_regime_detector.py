...
class MarketRegimeDetector:
    ...
    def analyze_market(
        ...
        if current_idx < max(self.config.ma_lookback_slow, self.config.support_resistance_lookback):
            return self._default_analysis()
            
        # Get current and historical data
        current_bar = df.iloc[current_idx]
        lookback_data = df.iloc[max(0, current_idx - self.config.support_resistance_lookback):current_idx + 1]
        ...
        is_breakout = self._detect_breakout(lookback_data, current_bar, support_resistance)
    ...
    def _detect_market_regime(
        ...
        if 'sma_20' not in current_bar:
            sma_fast = data['close'].rolling(self.config.ma_lookback_fast).mean().iloc[-1]
            sma_slow = data['close'].rolling(self.config.ma_lookback_slow).mean().iloc[-1]
        else:
        ...
        # Check for choppy market
            ...
            high_low_ratio = data['high'].rolling(20).max() / data['low'].rolling(20).min()
    ...
    def _detect_volatility_regime(
        ...
        if len(self.volatility_history) > 50:
            ...
            avg_atr = np.mean(self.volatility_history) if self.volatility_history else current_atr
    ...
    def _find_support_resistance(
        ...
        for i in range(-5, 6):
            level = round(current_price / round_interval) * round_interval + i * round_interval
    ...
    def _evaluate_trading_conditions(
        ...
        if market_regime == MarketRegime.RANGING:
            if volatility_regime not in [VolatilityRegime.NORMAL, VolatilityRegime.HIGH]:
                return False
