...
class LLMClient:
                ...
                with open('.env', 'r') as f:
                    for line in f:
                        if line.startswith('LLM_MODEL='):
                            llm_model = line.strip().split('=', 1)[1].split('#')[0].strip()
                            break
            except Exception:
    ...
    def _get_embedding_openai(self, text: str, model: Optional[str] = None) -> List[float]:
        ...
        Returns:
            List of floats representing the embedding vector
        """
        embedding_model = model or os.environ.get("EMBEDDING_MODEL", "text-embedding-3-large")
    ...
    async def generate_multimodal_response(
        ...
        if self.provider == "openai":
            ...
            return await self.generate_text_async(prompt, system_prompt, temperature, max_tokens)
