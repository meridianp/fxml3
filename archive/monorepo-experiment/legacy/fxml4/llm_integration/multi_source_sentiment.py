...
class MultiSourceSentimentAggregator:
    ...
    async def get_aggregated_sentiment(self, 
                                     symbol: str,
        ...
        sources_tasks = [
            ...
            self._get_alpha_vantage_sentiment(symbol) if self.alpha_vantage_key else None,
        ...
        return {
            ...
            'confidence': np.mean([s['confidence'] for s in sentiment_scores]) if sentiment_scores else 0.3,
            'source_sentiments': source_sentiments,
            'llm_consensus': llm_consensus,
            'total_articles': len(all_articles),
            'recent_headlines': [a['title'] for a in sorted(
                all_articles, 
    ...
    async def _get_reuters_sentiment(self, symbol: str, lookback_hours: int) -> Dict[str, Any]:
            ...
            for feed_url in feeds:
                ...
                if feed and feed.entries:
                    for entry in feed.entries[:10]:
                        ...
                        if self._is_relevant_to_symbol(entry.title + entry.get('summary', ''), symbol):
                            articles.append({
                                'title': entry.title,
                                'summary': entry.get('summary', ''),
                                'url': entry.link,
                                'timestamp': datetime.fromtimestamp(entry.published_parsed),
                                'source': 'reuters'
                            })
            
            # Analyze sentiment
            if articles:
                sentiment_score = await self._analyze_articles_sentiment(articles, symbol)
                return {
                    'source': 'reuters',
    ...
    async def _get_yahoo_finance_sentiment(self, symbol: str, lookback_hours: int) -> Dict[str, Any]:
    ...
    async def _get_marketwatch_sentiment(self, symbol: str, lookback_hours: int) -> Dict[str, Any]:
                ...
                async with session.get(url, params=params) as response:
                    if response.status == 200:
                        ...
                        for item in news_items[:10]:
                            if self._is_relevant_to_symbol(item.get('headline', ''), symbol):
                                articles.append({
    ...
    def _is_relevant_to_symbol(self, text: str, symbol: str) -> bool:
        ...
        pair_mentioned = symbol.lower() in text_lower or f"{base_currency}/{quote_currency}".lower() in text_lower
        
        return pair_mentioned or (base_mentioned and quote_mentioned) or base_mentioned
    
    async def _analyze_articles_sentiment(self, articles: List[Dict], symbol: str) -> float:
...
Respond with just a number between -1 and 1."""
    ...
    async def _get_llm_consensus(self, source_sentiments: Dict[str, Any], symbol: str) -> Dict[str, Any]:
