...
class MultiTimeframeChartValidator:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.candles_per_timeframe = config.get('candles_per_timeframe', {
    ...
    async def validate_trading_signal_mtf(self,
                                        signal: Dict[str, Any],
                                        price_data_dict: Dict[str, pd.DataFrame],
                                        indicators_dict: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _generate_mtf_chart(self,
                          price_data_dict: Dict[str, pd.DataFrame],
                          indicators_dict: Dict[str, Dict[str, Any]],
                          signal: Dict[str, Any]) -> Image.Image:
        ...
        for idx, timeframe in enumerate(self.timeframes):
            ...
            if 'support_levels' in indicators:
                for level in indicators['support_levels'][-3:]:
                    ax_main.axhline(y=level, color='green', alpha=0.3, linestyle='--', linewidth=1)
            
            if 'resistance_levels' in indicators:
                for level in indicators['resistance_levels'][-3:]:
                    ax_main.axhline(y=level, color='red', alpha=0.3, linestyle='--', linewidth=1)
            
            # Add title for this timeframe
            current_price = plot_data['close'].iloc[-1]
            price_change = (plot_data['close'].iloc[-1] / plot_data['close'].iloc[0] - 1) * 100
            ...
            if idx < num_timeframes - 1:
                ...
                ax_main.annotate('', xy=(1.02, 0.5), xytext=(1.02, -0.5),
    ...
    def _annotate_signal_mtf(self, ax, price_data: pd.DataFrame, signal: Dict[str, Any]):
        ...
        if signal.get('direction') == 'BUY':
            ax.scatter(last_idx, last_price * 0.998, 
                      marker='^', color='green', s=200, zorder=5)
            ax.text(last_idx, last_price * 0.995, 'BUY', 
                   ha='center', va='top', fontsize=10, 
                   color='green', weight='bold')
        elif signal.get('direction') == 'SELL':
            ax.scatter(last_idx, last_price * 1.002, 
                      marker='v', color='red', s=200, zorder=5)
            ax.text(last_idx, last_price * 1.005, 'SELL', 
                   ha='center', va='bottom', fontsize=10, 
                   color='red', weight='bold')
        
        # Entry, stop loss, take profit levels
        if signal.get('entry_price'):
            ax.axhline(y=signal['entry_price'], color='yellow', 
                      linestyle='-', linewidth=2, alpha=0.7)
            ax.text(last_idx + 1, signal['entry_price'], 
                   f"Entry: {signal['entry_price']:.5f}",
                   color='yellow', fontsize=9, va='center')
        
        if signal.get('stop_loss'):
            ax.axhline(y=signal['stop_loss'], color='red',
                      linestyle='--', linewidth=2, alpha=0.7)
            ax.text(last_idx + 1, signal['stop_loss'],
                   f"SL: {signal['stop_loss']:.5f}",
                   color='red', fontsize=9, va='center')
        
        if signal.get('take_profit'):
            ax.axhline(y=signal['take_profit'], color='green',
                      linestyle='--', linewidth=2, alpha=0.7)
            ax.text(last_idx + 1, signal['take_profit'],
    ...
    async def _validate_with_llm_mtf(self,
                                   chart_base64: str,
                                   signal: Dict[str, Any],
                                   price_data_dict: Dict[str, pd.DataFrame],
                                   indicators_dict: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
...
- overall_assessment: comprehensive summary"""
    ...
    def _fallback_validation_mtf(self, signal: Dict[str, Any], 
                                indicators_dict: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _calculate_enhanced_confidence(self,
                                     ...
                                     timeframe_alignment: float) -> float:
        ...
        # Apply penalties
        if min(ml_confidence, llm_confidence, pattern_clarity, timeframe_alignment) < 0.3:
        ...
        if all(score > 0.7 for score in [ml_confidence, llm_confidence, pattern_clarity, timeframe_alignment]):
