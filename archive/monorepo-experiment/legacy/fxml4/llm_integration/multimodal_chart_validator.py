...
class MultiModalChartValidator:
    ...
    async def validate_trading_signal(self,
                                    signal: Dict[str, Any],
        ...
        try:
            ...
            chart_image = self._generate_analysis_chart(
                price_data,
                indicators,
                signal
            )
    ...
    def _generate_analysis_chart(self,
                               price_data: pd.DataFrame,
        ...
        if 'support_levels' in indicators:
            for level in indicators['support_levels'][-3:]:  # Last 3 levels
                ax1.axhline(y=level, color='green', alpha=0.3, linestyle='--', linewidth=1)
                ax1.text(len(plot_data)-1, level, f'S: {level:.5f}', 
        ...
        if 'resistance_levels' in indicators:
            for level in indicators['resistance_levels'][-3:]:  # Last 3 levels
                ax1.axhline(y=level, color='red', alpha=0.3, linestyle='--', linewidth=1)
                ax1.text(len(plot_data)-1, level, f'R: {level:.5f}', 
    ...
    def _annotate_signal(self, ax, price_data: pd.DataFrame, signal: Dict[str, Any]):
        ...
        if signal.get('stop_loss') and signal.get('take_profit'):
            ...
            # Add R:R text
            ax.text(last_idx - 10, price_data['high'].max() * 0.99,
                   f"R:R = 1:{rr_ratio:.1f}",
    ...
    def _annotate_patterns(self, ax, price_data: pd.DataFrame, patterns: List[Dict]):
        ...
        for pattern in patterns[-3:]:  # Show last 3 patterns
            if pattern.get('start_idx') and pattern.get('end_idx'):
                ...
                end_idx = min(len(price_data)-1, pattern['end_idx'] - (len(price_data) - 100))
    ...
    async def _validate_with_llm(self,
        ...
        price_change_1d = (price_data['close'].iloc[-1] / price_data['close'].iloc[-24] - 1) * 100 if len(price_data) > 24 else 0
...
- overall_assessment: brief summary"""
    ...
    def _fallback_validation(self, signal: Dict[str, Any], indicators: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Check risk/reward
        if signal.get('stop_loss') and signal.get('take_profit') and signal.get('entry_price'):
    ...
    def _calculate_enhanced_confidence(self,
        ...
        if all(score > 0.7 for score in [ml_confidence, llm_confidence, pattern_clarity]):
