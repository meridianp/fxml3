...
class CandleBuilder:
    ...
    def __init__(self, timeframe_minutes: int = 1):
        ...
        logger.info(f"Initialized CandleBuilder with timeframe: {timeframe_minutes} minutes")
    
    def add_tick(self, symbol: str, timestamp: datetime, price: float, size: float) -> Optional[Dict[str, Any]]:
        ...
        if symbol not in self.symbols or 'candle_start' not in self.symbols[symbol] or candle_start != self.symbols[symbol]['candle_start']:
    ...
    def _complete_candle(self, symbol: str) -> Dict[str, Any]:
        ...
        logger.debug(f"Completed candle for {symbol} at {candle['candle_start']}: " +
    ...
    def get_completed_candles(self, symbol: str, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        return sorted(symbol_candles, key=lambda x: x['timestamp'], reverse=True)[:limit]
    
    def get_completed_candles_as_dataframe(self, symbol: str, limit: int = 100) -> pd.DataFrame:
        ...
        if not candles:
            ...
            return pd.DataFrame(columns=['symbol', 'timestamp', 'open', 'high', 'low', 'close', 'volume', 'tick_count'])
    ...
    def clear_candles(self, older_than: Optional[datetime] = None):
        ...
        self.completed_candles = [c for c in self.completed_candles if c['timestamp'] > older_than]
...
class TickAggregator:
    ...
    def process_tick(self, symbol: str, timestamp: datetime, price: float, size: float = np.nan, 
                     tick_type: str = "trade", source: str = "ib",
                     store_in_db: bool = False) -> Dict[int, Optional[Dict[str, Any]]]:
        ...
        if self.tick_count % 1000 == 0:
            logger.info(f"Processed {self.tick_count} ticks, last symbol: {symbol}, time: {timestamp}")
        
        return completed_candles
    
    def get_latest_candle(self, symbol: str, timeframe: int = 1, include_current: bool = False) -> Optional[Dict[str, Any]]:
    ...
    def get_candles(self, symbol: str, timeframe: int = 1, limit: int = 100) -> pd.DataFrame:
        ...
        return self.candle_builders[timeframe].get_completed_candles_as_dataframe(symbol, limit)
