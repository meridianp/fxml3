...
class RobustModelTrainer:
    ...
    def validate_no_lookahead(
        ...
        """
        suspicious_keywords = [
            'future', 'next', 'forward', 'ahead', 'later',
        ...
        for feature in features:
            has_bias = any(keyword in feature.lower() for keyword in suspicious_keywords)
        ...
        if removed:
            logger.warning(f"Removed {len(removed)} features with potential look-ahead bias: {removed[:5]}...")
    ...
    def train_robust_model(
        ...
        logger.info(f"Selected {len(selected_features)} stable features from {len(clean_features)}")
