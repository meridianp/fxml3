...
def create_basic_technical_features(
    ...
    if indicators is None:
        ...
        indicators = ["sma", "ema", "rsi", "bollinger", "macd", "stoch", "atr", "adx", "zigzag"]
    ...
    if "macd" in indicators:
        ...
        # Calculate MACD
        ema_fast = df["close"].ewm(span=fast_period, min_periods=fast_period, adjust=False).mean()
        ema_slow = df["close"].ewm(span=slow_period, min_periods=slow_period, adjust=False).mean()
        df["macd"] = ema_fast - ema_slow
        df["macd_signal"] = df["macd"].ewm(span=signal_period, min_periods=signal_period, adjust=False).mean()
    ...
    df["weekly_change"] = (df["close"] - df["close"].shift(5 * 24)) / df["close"].shift(5 * 24) * 100  # 5 days' worth of hourly data
    
    # Fill NA values if requested
    if fillna:
        df = df.ffill().bfill()  # Use new recommended methods instead of deprecated fillna(method=)
    
    # Add enhanced pivot-related features if requested
    if add_enhanced_features and any(col for col in df.columns if col.endswith(('_r1', '_s1', '_r2', '_s2'))):
        # Add pivot breakout indicators
        pivot_cols = [col for col in df.columns if any(x in col for x in ["_r1", "_s1", "_r2", "_s2"])]
        for col in pivot_cols:
            if "r1" in col:
                # Resistance breakout
                prefix = col.split("_r1")[0]
                if f"{prefix}_r1" in df.columns:
                    df[f"{prefix}_breakout_up"] = (df["close"] > df[f"{prefix}_r1"]).astype(int)
            elif "s1" in col:
                # Support breakout
                prefix = col.split("_s1")[0]
                if f"{prefix}_s1" in df.columns:
                    df[f"{prefix}_breakout_down"] = (df["close"] < df[f"{prefix}_s1"]).astype(int)
...
def create_technical_features(
    ...
    try:
        ...
        logger.warning(f"Using fallback technical indicators due to pandas_ta import issue: {str(e)}")
        return create_basic_technical_features(
            data, indicators, ma_periods, include_original, fillna, add_enhanced_features)
    except Exception as e:
        logger.warning(f"Error importing pandas_ta: {str(e)}, using fallback indicators")
        return create_basic_technical_features(
            data, indicators, ma_periods, include_original, fillna, add_enhanced_features)
    ...
    if indicators is None:
        ...
        indicators = ["sma", "ema", "rsi", "macd", "bollinger", "stoch", "atr", 
                     "adx", "zigzag", "pivot_points", "session_indicators"]
        ...
        for col in pivot_cols:
            ...
            if "r2" in col:
                ...
                if f"{prefix}_r2" in df.columns:
                    df[f"{prefix}_breakout_r2"] = (df["close"] > df[f"{prefix}_r2"]).astype(int)
            elif "s2" in col:
                prefix = col.split("_s2")[0]
                if f"{prefix}_s2" in df.columns:
                    df[f"{prefix}_breakout_s2"] = (df["close"] < df[f"{prefix}_s2"]).astype(int)
...
def create_target_labels(
    ...
    if method == 'volatility_adjusted' or volatility_adjusted:
        ...
        if trend_adjusted:
            ...
            trend = df["close"].rolling(window=trend_window).mean().pct_change(trend_window // 2)
            max_trend = trend.abs().quantile(0.95)  # Use 95th percentile to avoid extreme values
        ...
        if n_classes == 3:
            ...
            upper_rolling = future_return.rolling(window=window, min_periods=window//2).quantile(upper_q)
            lower_rolling = future_return.rolling(window=window, min_periods=window//2).quantile(lower_q)
            ...
            rolling_median = future_return.rolling(window=window, min_periods=window//2).median()
            target = np.zeros(len(future_return))
            for i in range(len(future_return)):
...
def calculate_weekly_pivot_points(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in pivot_cols:
        result_df[f'distance_to_{col}'] = (result_df[col] / result_df['close'] - 1) * 100
    ...
    result_df['between_S1_R1'] = ((result_df['close'] >= result_df['S1']) & 
                                  (result_df['close'] <= result_df['R1'])).astype(int)
    
    # Calculate if price is near a pivot point (within 0.1%)
    for col in pivot_cols:
        result_df[f'near_{col}'] = (abs(result_df['close'] - result_df[col]) / result_df['close'] < 0.001).astype(int)
...
def identify_trading_sessions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for session, (start_hour, end_hour) in [
        ...
        if start_hour < end_hour:
            ...
            open_times = pd.to_datetime(date_str + ' ' + f"{start_hour:02d}:00:00", utc=True)
            close_times = pd.to_datetime(date_str + ' ' + f"{end_hour:02d}:00:00", utc=True)
        else:
            # Overnight case (e.g., Sydney 21-6)
            open_times = pd.to_datetime(date_str + ' ' + f"{start_hour:02d}:00:00", utc=True)
            
            # For close time, use next day
            next_day = (pd.to_datetime(date_str, utc=True) + pd.Timedelta(days=1)).strftime('%Y-%m-%d')
            close_times = pd.to_datetime(next_day + ' ' + f"{end_hour:02d}:00:00", utc=True)
...
def calculate_session_pivot_levels(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for session in ['london', 'newyork', 'tokyo', 'sydney']:
        ...
        for i, start_idx in enumerate(session_starts):
            ...
            prev_session = result_df.loc[prev_start:start_idx][result_df[session_col] == 1]
    ...
    df_daily['daily_r3'] = df_daily['high'] + 2 * (df_daily['daily_pp'] - df_daily['low'])
    df_daily['daily_s3'] = df_daily['low'] - 2 * (df_daily['high'] - df_daily['daily_pp'])
    
    # Forward fill to original index
    pivot_cols = ['daily_pp', 'daily_r1', 'daily_s1', 'daily_r2', 'daily_s2', 'daily_r3', 'daily_s3']
