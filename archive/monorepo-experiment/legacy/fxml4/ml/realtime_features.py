...
from fxml4.ml.features import calculate_technical_features, add_pivot_points, add_session_features
...
class RealtimeFeatureGenerator:
    ...
    def __init__(
        ...
        self.prediction_horizon = feature_config.get("prediction_horizon", 12)  # Default 12 periods
        ...
        logger.info(f"Initialized RealtimeFeatureGenerator for {self.symbol} on {self.timeframe} timeframe")
    ...
    def _on_daily_candle_update(self, symbol: str, timeframe: str, candles: pd.DataFrame):
        """
        ...
        try:
            ...
            with self.feature_lock:
                if symbol not in self.raw_data or self.raw_data[symbol].empty:
                    logger.warning(f"No raw data available for {symbol}, skipping feature generation")
                    return
            ...
            if len(data) < self.lookback:
                logger.warning(f"Not enough data for {symbol}: {len(data)} < {self.lookback}, skipping feature generation")
                return
            ...
            if self.model_feature_list:
                ...
                missing_features = set(self.model_feature_list) - set(feature_df.columns)
                if missing_features:
                ...
                available_features = [col for col in self.model_feature_list if col in feature_df.columns]
            ...
            if self.scaler:
                ...
                try:
                    ...
                    feature_df = pd.DataFrame(scaled_values, index=feature_df.index, columns=feature_df.columns)
            ...
            logger.debug(f"Generated {len(feature_df.columns)} features for {symbol} at {now}")
    ...
    def get_features_history(self, symbol: str = None, n_periods: int = 10) -> pd.DataFrame:
        """
    ...
    def unregister_feature_callback(self, callback: Callable[[str, pd.DataFrame], None]):
        """
...
class StreamingPredictor:
    ...
    def _register_feature_callbacks(self):
        ...
        if not self.feature_generator:
            logger.error("No RealtimeFeatureGenerator provided, cannot register callbacks")
            return
    ...
    def _generate_prediction(self, feature_df: pd.DataFrame):
        ...
        try:
            ...
            logger.debug(f"Generated prediction for {self.symbol}: class={prediction_class}, confidence={confidence:.2f}")
    ...
    def unregister_prediction_callback(self, callback: Callable[[Dict[str, Any]], None]):
        """
    ...
    def stop(self):
        ...
        if self.feature_generator:
            try:
                self.feature_generator.unregister_feature_callback(self._on_feature_update)
