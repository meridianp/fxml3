...
class WalkForwardOptimizer:
        ...
        while current_date < end_date:
            fold = {
                ...
                'validation': (
                    ...
                    current_date + timedelta(days=self.purge_days + self.validation_days)
                ),
                'test': (
                    current_date + timedelta(days=self.purge_days + self.validation_days + self.purge_days),
                    current_date + timedelta(days=self.purge_days + self.validation_days + self.purge_days + self.test_days)
                )
            }
    ...
    def run_backtest(
        ...
        for i, fold in enumerate(folds):
            ...
            train_mask = (data.index >= fold['train'][0]) & (data.index <= fold['train'][1])
            val_mask = (data.index >= fold['validation'][0]) & (data.index <= fold['validation'][1])
            test_mask = (data.index >= fold['test'][0]) & (data.index <= fold['test'][1])
            ...
            test_proba = best_model.predict_proba(X_test_scaled) if hasattr(best_model, 'predict_proba') else None
            ...
            for j, (idx, pred) in enumerate(zip(X_test.index, test_pred)):
                results['predictions'].append({
                    ...
                    'fold': i,
                    'prediction': pred,
                    'probability': test_proba[j].max() if test_proba is not None else None,
        ...
        if not all_predictions.empty:
            ...
            results['aggregate_metrics'] = {
                ...
                'accuracy': accuracy_score(all_predictions['actual'], all_predictions['prediction']),
                'precision': precision_score(all_predictions['actual'], all_predictions['prediction'], 
                                           average='weighted', zero_division=0),
                'recall': recall_score(all_predictions['actual'], all_predictions['prediction'], 
                                     average='weighted', zero_division=0),
                'avg_fold_accuracy': np.mean([f['test_accuracy'] for f in results['folds']]),
                'std_fold_accuracy': np.std([f['test_accuracy'] for f in results['folds']])
            }
            ...
            results['parameter_stability'] = {
                ...
                'most_common_params': max(param_counts.items(), key=lambda x: x[1])[0] if param_counts else None,
                'parameter_consistency': max(param_counts.values()) / len(results['parameters']) if results['parameters'] else 0
            }
