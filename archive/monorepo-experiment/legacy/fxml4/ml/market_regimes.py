...
class MarketRegimeClassifier:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.n_regimes = self.regime_config.get(
            "n_regimes", 
            get_config("ml.market_regimes.n_regimes", 5)
        )
        self.use_economic_data = self.regime_config.get(
            "use_economic_data", 
            get_config("ml.market_regimes.use_economic_data", True)
        )
        self.use_pca = self.regime_config.get(
            "use_pca", 
            get_config("ml.market_regimes.use_pca", True)
        )
        self.pca_components = self.regime_config.get(
            "pca_components", 
            get_config("ml.market_regimes.pca_components", 5)
        )
        self.regime_window = self.regime_config.get(
            "regime_window", 
            get_config("ml.market_regimes.regime_window", 126)  # ~6 months
        )
        
        # Features to use for regime classification
        self.market_features = self.regime_config.get("market_features", [
        ...
        if self.use_economic_data:
            self.economic_features = self.regime_config.get("economic_features", [
                "econ_VIXCLS",             # VIX
        ...
        logger.info(f"Initialized market regime classifier with {self.n_regimes} regimes")
    
    def fit(self, market_data: pd.DataFrame, economic_data: Optional[pd.DataFrame] = None) -> None:
        ...
        if self.use_pca and X.shape[1] > self.pca_components:
            ...
            X_pca = pd.DataFrame(
                self.pca_model.fit_transform(X_scaled),
        ...
        if self.use_pca:
            ...
            self.regime_centers = self.scaler.inverse_transform(self.kmeans_model.cluster_centers_)
        
        logger.info(f"Fitted market regime classifier with {self.n_regimes} regimes")
    
    def predict(self, market_data: pd.DataFrame, economic_data: Optional[pd.DataFrame] = None) -> pd.Series:
    ...
    def get_regime_characteristics(self) -> pd.DataFrame:
        ...
        if self.regime_labels is not None:
            ...
            centers_df["frequency"] = [regime_pcts.get(i, 0) for i in range(self.n_regimes)]
            centers_df["count"] = [regime_counts.get(i, 0) for i in range(self.n_regimes)]
    ...
    def describe_regimes(self) -> Dict[int, Dict[str, Any]]:
        ...
        for i in range(self.n_regimes):
            ...
            if self.use_economic_data and isinstance(description["economic_context"], dict):
                ...
                if econ:
                    ...
                    if "interest_rates" in econ:
                        context_points.append(f"{econ['interest_rates']} interest rates")
                    if "growth" in econ:
                    ...
                    if "market_anxiety" in econ:
                        context_points.append(f"{econ['market_anxiety']} market anxiety")
    ...
    def _prepare_feature_matrix(
        ...
        market_features = market_data[self.market_features].copy() if all(
            f in market_data.columns for f in self.market_features
        ...
        if self.use_economic_data and economic_data is not None and not economic_data.empty:
            ...
            if any(col.startswith('econ_') for col in market_data.columns):
                ...
                econ_cols = [col for col in self.economic_features if col in market_data.columns]
                if econ_cols:
                    economic_features = market_data[econ_cols].copy()
            else:
                # Need to create economic features
                processed_economic = self.economic_engineer.create_features(economic_data)
                ...
                if not isinstance(processed_economic.index, pd.DatetimeIndex):
                    ...
                    date_range = pd.date_range(
                        start=min(market_data.index.min(), processed_economic.index.min()),
                        end=max(market_data.index.max(), processed_economic.index.max()),
                    ...
                    aligned_economic = daily_economic.reindex(market_data.index, method='ffill')
                    
                    # Add prefix to columns
                    aligned_economic = aligned_economic.add_prefix('econ_')
                    
                    # Extract features if they exist
                    econ_cols = [col for col in self.economic_features if col in aligned_economic.columns]
                    if econ_cols:
        ...
        if X.isna().any().any():
            logger.warning(f"Feature matrix contains {X.isna().sum().sum()} NaN values after forward/backward fill")
            X = X.dropna()
