...
def extract_candlestick_patterns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        result_df["body_size_relative"] = result_df["body_size"] / (result_df["high"] - result_df["low"])
        ...
        result_df["upper_shadow_ratio"] = result_df["upper_shadow"] / result_df["body_size"].replace(0, np.nan)
        result_df["lower_shadow_ratio"] = result_df["lower_shadow"] / result_df["body_size"].replace(0, np.nan)
        
        # Candlestick direction
        result_df["candle_direction"] = np.where(result_df["close"] >= result_df["open"], 1, -1)
        
        # Range (high - low)
        result_df["range"] = result_df["high"] - result_df["low"]
        
        # Relative position of close within day's range
        result_df["close_position"] = (result_df["close"] - result_df["low"]) / result_df["range"].replace(0, np.nan)
...
def extract_fibonacci_features(df: pd.DataFrame, window_size: int = 20) -> pd.DataFrame:
    ...
    for level in fib_levels:
        ...
        result_df[f"fib_high_{int(level*1000)}"] = result_df["local_high"] - (result_df["local_range"] * level)
        # Retracement from low
        result_df[f"fib_low_{int(level*1000)}"] = result_df["local_low"] + (result_df["local_range"] * level)
    
    # Distance of current price to Fibonacci levels
    for level in fib_levels:
        # Distance to retracement from high
        result_df[f"dist_fib_high_{int(level*1000)}"] = abs(result_df["close"] - result_df[f"fib_high_{int(level*1000)}"])
        # Distance to retracement from low
        result_df[f"dist_fib_low_{int(level*1000)}"] = abs(result_df["close"] - result_df[f"fib_low_{int(level*1000)}"])
    
    # Proximity to any Fibonacci level
    all_fib_columns = [f"fib_high_{int(level*1000)}" for level in fib_levels] + [f"fib_low_{int(level*1000)}" for level in fib_levels]
    
    # Calculate minimum distance to any Fibonacci level
    result_df["min_fib_distance"] = result_df.apply(
        lambda row: min([abs(row["close"] - row[col]) for col in all_fib_columns]), axis=1
    )
    ...
    result_df["relative_position"] = (result_df["close"] - result_df["local_low"]) / result_df["local_range"].replace(0, np.nan)
...
def extract_trend_features(df: pd.DataFrame, periods: List[int] = None) -> pd.DataFrame:
    ...
    try:
        ...
        for period in periods:
            ...
            result_df[f"price_rel_sma_{period}"] = result_df["close"] / result_df[f"sma_{period}"] - 1
        ...
        for i, fast_period in enumerate(periods[:-1]):
            for slow_period in periods[i+1:]:
                result_df[f"sma_{fast_period}_cross_sma_{slow_period}"] = np.where(
                    result_df[f"sma_{fast_period}"] > result_df[f"sma_{slow_period}"], 1, -1
                )
        
        # Slope of moving averages
        for period in periods:
            # Calculate the slope of SMA using rolling regression
            result_df[f"sma_{period}_slope"] = ta.slope(result_df[f"sma_{period}"], length=5)
        ...
        # ADX (trend strength)
        adx = ta.adx(high=result_df["high"], low=result_df["low"], close=result_df["close"], length=14)
...
def extract_wave_features(df: pd.DataFrame, min_length: int = 5, max_length: int = 50) -> pd.DataFrame:
    ...
    for n in look_periods:
        ...
        for _, wave in wave_df.iterrows():
            ...
            if f"corrective_wave_{n}" in wave and not pd.isna(wave[f"corrective_wave_{n}"]):
