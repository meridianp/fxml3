...
def plot_candlestick_chart(
    ...
    # Up days
    price_ax.bar(up.index, up["close"] - up["open"], width, bottom=up["open"], color="green", alpha=0.5)
    price_ax.bar(up.index, up["high"] - up["close"], width2, bottom=up["close"], color="green", alpha=0.5)
    price_ax.bar(up.index, up["open"] - up["low"], width2, bottom=up["low"], color="green", alpha=0.5)
    
    # Down days
    price_ax.bar(down.index, down["open"] - down["close"], width, bottom=down["close"], color="red", alpha=0.5)
    price_ax.bar(down.index, down["high"] - down["open"], width2, bottom=down["open"], color="red", alpha=0.5)
    price_ax.bar(down.index, down["close"] - down["low"], width2, bottom=down["low"], color="red", alpha=0.5)
...
def plot_interactive_chart(
    ...
    if show_waves:
        wave_cols = [col for col in df.columns if col.startswith("impulse_wave_") or col.startswith("corrective_wave_")]
                    ...
                    if wave_num in wave_values:
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                mode="markers",
                                marker=dict(
                                    symbol="triangle-down" if wave_num % 2 == 0 else "triangle-up",
                                    size=10,
