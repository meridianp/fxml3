...
from fxml3.strategy.entry_signals import EntrySignal, SignalType, SignalStrength
from fxml3.strategy.risk_management import RiskManager
from fxml3.strategy.position_sizing import PositionSizer, PositionSizingMethod, PositionSize
...
class PortfolioManager:
    ...
    def evaluate_new_position(
        ...
        if symbol in self.positions and self.positions[symbol]:
            for position in self.positions[symbol]:
                if (position.type == "long" and entry_signal.signal_type == SignalType.LONG) or \
                   (position.type == "short" and entry_signal.signal_type == SignalType.SHORT):
                    evaluation["reasons"].append(f"Already have a {position.type} position for {symbol}")
    ...
    def add_position(
        ...
        position_type = "long" if entry_signal.signal_type == SignalType.LONG else "short"
        ...
        for symbol, positions in self.positions.items():
            for i, position in enumerate(positions):
                if position.id == position_id:
                    ...
                    if updates.get('status') == 'partial_exit' and 'exit_data' in updates:
    ...
    def get_portfolio_stats(self, account_size: float) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "risk_utilization": self.total_portfolio_risk / self.max_portfolio_risk if self.max_portfolio_risk > 0 else 0,
            ...
            "position_count_by_symbol": {symbol: len(positions) for symbol, positions in self.positions.items()},
            "risk_by_asset_class": {
    ...
    def _update_exposure(self, symbol: str, position: Position, add: bool = True) -> None:
        """
        ...
        self.current_exposure[symbol] = self.current_exposure.get(symbol, 0.0) + risk_percentage
        ...
        if asset_class in self.exposure_by_asset_class and self.exposure_by_asset_class[asset_class] <= 0:
            del self.exposure_by_asset_class[asset_class]
    
    def _update_correlation_matrix(self, historical_data: Dict[str, pd.DataFrame]) -> None:
        """
        ...
        for symbol, data in historical_data.items():
            if len(data) >= self.correlation_lookback:
                valid_symbols[symbol] = data['close'].tail(self.correlation_lookback).values
    ...
    def _initialize_asset_classes(self) -> None:
        ...
        minor_pairs = ["EURGBP", "EURJPY", "EURCHF", "EURAUD", "EURCAD", "GBPJPY", "GBPAUD", "GBPCAD", "CADJPY"]
    ...
    def analyze_portfolio_correlation_risk(self) -> Dict[str, Any]:
        ...
        high_risk_groups = {
            group: risk for group, risk in group_risk.items()
        ...
        for group_name, positions in correlation_groups.items():
            long_risk = sum(pos.risk_percentage for pos in positions if pos.type == "long")
            short_risk = sum(pos.risk_percentage for pos in positions if pos.type == "short")
