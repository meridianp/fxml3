...
class PositionSizer:
    ...
    def calculate_position_size(
        ...
        signal_take_profit = entry_signal.take_profit if take_profit is None else take_profit
        ...
        if method == PositionSizingMethod.FIXED_RISK:
            return self._fixed_risk_position_size(
                entry_price, signal_stop_loss, signal_take_profit, 
                account_size, entry_signal
            )
            
        elif method == PositionSizingMethod.KELLY:
            return self._kelly_position_size(
                entry_price, signal_stop_loss, signal_take_profit, 
                account_size, entry_signal, custom_data
            )
    ...
    def _kelly_position_size(
        ...
        # Otherwise, estimate from the risk/reward ratio
            ...
            win_rate = 0.4 + (entry_signal.confidence * 0.4)  # 0.4 to 0.8 based on confidence
        ...
        return PositionSize(
            ...
            metadata={
                ...
                "full_kelly": kelly_f / self.kelly_fraction if self.kelly_fraction > 0 else 0
            }
        )
    ...
    def _scaling_position_size(
        ...
        price_risk = abs(entry_price - stop_loss)
        initial_position_size = initial_risk_amount / price_risk if price_risk > 0 else 0
        ...
        if remaining_levels > 0:
            ...
            level_percents = self._calculate_scaling_levels(remaining_percent, remaining_levels)
    ...
    def _calculate_win_metrics_from_history(self) -> Tuple[float, float]:
        ...
        avg_loss = total_loss / (len(self.trade_history) - wins) if (len(self.trade_history) - wins) > 0 else 1.0
