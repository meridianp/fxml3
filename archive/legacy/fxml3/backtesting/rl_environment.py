...
class ForexTradingEnv(gym.Env):
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        account_features = np.array([
            ...
            sum(self.returns_history[-10:]) if len(self.returns_history) >= 10 else 0,  # Recent returns
        ])
    ...
    def _extract_wave_features(self, idx: int) -> np.ndarray:
        ...
        total_length = relevant_pattern.get("end_idx", 0) - relevant_pattern.get("start_idx", 0)
    ...
    def _extract_sentiment_features(self, idx: int) -> np.ndarray:
        ...
        closest_idx = self.sentiment_data.index.get_indexer([timestamp], method="nearest")[0]
