...
def create_synthetic_elliott_wave_data() -> pd.DataFrame:
    ...
    df['high'] = df.apply(
        lambda row: max(row['open'], row['close']) * (1 + np.random.uniform(0.001, daily_volatility)), 
        axis=1
    )
    df['low'] = df.apply(
        lambda row: min(row['open'], row['close']) * (1 - np.random.uniform(0.001, daily_volatility)), 
        axis=1
    )
...
def analyze_and_plot(df: pd.DataFrame, output_path: Optional[str] = None) -> None:
    ...
    labeled_df = fib_calculator.calculate_fibonacci_price_zones(labeled_df, window_size=21)
    ...
    for pattern in pattern_types:
        pattern_counts[pattern] = sum(1 for label in wave_points.keys() if label.startswith(pattern))
        ...
        try:
            ...
            for period in [5, 8, 13, 21]:
                ...
                if pattern_col in labeled_df.columns and labeled_df[pattern_col].any():
                    ...
                    fig = plot_wave_analysis(labeled_df, wave_col=pattern_col, title=f"Elliott Wave Analysis - {pattern_col}")
