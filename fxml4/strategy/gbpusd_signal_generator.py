...
class GBPUSDSignalGenerator(SignalGenerator):
    ...
    def __init__(
        ...
        if model is not None:
            ...
            try:
                model_config = get_config().get("ml.gbpusd_model", {})
        ...
        self.signal_cooldown = self.config.get(
        ...
        if self.model is not None:
            logger.info(
                f"Initialized GBP/USD signal generator with model: {self.model.name}"
            )
        else:
            logger.warning("GBP/USD signal generator initialized without a model")

    def generate_signals(self, data: pd.DataFrame, **kwargs: Any) -> List[Signal]:
        ...
        try:
            ...
            if signal_type in self.last_signal_time:
                ...
                if time_diff < self.signal_cooldown:
                    logger.debug(
                        f"Signal suppressed due to cooldown period ({time_diff}s < {self.signal_cooldown}s)"
                    )
                    return signals
    ...
    def backtest(
        self, data: pd.DataFrame, target_horizon: int = 12, **kwargs: Any
    ) -> pd.DataFrame:
        ...
        try:
            ...
            results["forward_return"] = (
                results["close"].shift(-target_horizon) / results["close"] - 1
            )
            ...
            results["cum_return"] = (
                1 + results["signal_return"].fillna(0)
            ).cumprod() - 1
...
def create_ensemble_generator(
    ...
    return GBPUSDEnsembleGenerator(
        generators=generators, weights=weights, config=ensemble_config
    )
...
class GBPUSDEnsembleGenerator(SignalGenerator):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized GBP/USD ensemble generator with {len(generators)} models"
        )
