...
class EnhancedWaveSignalGenerator(SignalGenerator):
    ...
    def _calculate_stop_loss_level(
        ...
        # Default case
        return (
            current_price * (1 - base_stop_pct / 100)
            if signal_type == SignalType.ENTRY_LONG
            else current_price * (1 + base_stop_pct / 100)
        )

    def _calculate_take_profit_levels(
        self, entry_price: float, stop_loss: float, signal_type: SignalType
    ) -> Dict[str, float]:
    ...
    def generate_signals(
        ...
        **kwargs: Any,
        ...
        symbol = kwargs.get(
            "symbol",
            (
                data.get("symbol", ["unknown"])[0]
                if "symbol" in data.columns
                else "unknown"
            ),
        )
        timeframe = kwargs.get(
            "timeframe",
            (
                data.get("timeframe", ["unknown"])[0]
        ...
        try:
            ...
            analysis_results = self.wave_validator.analyze_with_sentiment(
                price_data=data, news_data=news_data if use_news else None
            )
