...
class NewsEventFilter:
    ...
    def _load_cache(self) -> None:
        ...
        try:
            if os.path.exists(self.cache_file):
                ...
                ).total_seconds() > self.cache_duration * 3600:
    ...
    def _save_cache(self) -> None:
        ...
        try:
            ...
            events_str_keys = {
                date.isoformat(): events for date, events in self.events_by_date.items()
            }
    ...
    def fetch_forexfactory_calendar(
        ...
        filtered_events = [
            event for event in all_events if start_date <= event["datetime"] <= end_date
        ]
            ...
            if time_text and time_text != "All Day" and time_text != "Tentative":
                try:
                    ...
                    if ":" in time_text:
                        ...
                        hour = int(
                            time_text.replace("am", "").replace("pm", "").strip()
                        )
                        if hour < 12 and "pm" in time_text.lower():
                            hour += 12
    ...
    def update_calendar(self) -> bool:
        ...
        try:
            ...
            if (
                ...
                and (now - self.last_cache_update).total_seconds()
...
class SpreadAnalyzer:
    ...
    def update_stats(self, symbol: str, market_data: pd.DataFrame) -> None:
        ...
        spreads = (
            (recent_data["high"] - recent_data["low"]) / recent_data["close"] * 10000
        )  # In pips
    ...
    def is_spread_abnormal(
        ...
        if market_data is not None and (
            symbol not in self.spread_stats
            or (
                datetime.now()
                - self.spread_stats[symbol].get("last_update", datetime.min)
            ).total_seconds()
            > 3600
        ):
        ...
        current_spread = (
            (current_bar["high"] - current_bar["low"]) / current_bar["close"] * 10000
        )  # In pips
...
class IntegratedNewsFilter:
    ...
    def is_news_event_time(
        ...
        if self.scheduled_updates and self.last_scheduled_update:
            hours_since_update = (
                timestamp - self.last_scheduled_update
            ).total_seconds() / 3600
        ...
        for name, release_dates in self.fred_release_cache.items():
            for release in release_dates:
                ...
                try:
                    ...
                    buffer_before = release_time - timedelta(
                        minutes=self.forex_factory.event_buffer_before
                    )
                    buffer_after = release_time + timedelta(
                        minutes=self.forex_factory.event_buffer_after
                    )
                    ...
                    if buffer_before <= timestamp <= buffer_after:
                        ...
                        if self.forex_factory.currency_specific and symbol:
                            symbol_currencies = self.forex_factory.currency_map.get(
                                symbol, []
                            )
                    ...
                    logger.warning(
                        f"Error processing FRED release date {release_date_str}: {e}"
                    )
    ...
    def get_upcoming_events(
        ...
        ff_events = self.forex_factory.get_upcoming_events(
            from_time, days_ahead, symbol
        )
