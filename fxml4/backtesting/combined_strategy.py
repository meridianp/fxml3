...
from fxml4.backtesting.event import EventType, OrderEvent, SignalEvent
...
class TakeProfitManager:
    ...
    def __init__(self, default_profit_pct: float = 2.0, use_wave_targets: bool = False):
    ...
    def calculate_take_profit(
        self, entry_price: float, signal: SignalEvent, risk_amount: float = 0.0
    ) -> float:
...
class CombinedStrategy:
    ...
    def __init__(
        ...
        self.take_profit_manager = TakeProfitManager(
            default_profit_pct=3.0, use_wave_targets=self.use_wave_stops
        )
    ...
    def _should_ignore_signal(self, signal: Signal, current_time: pd.Timestamp) -> bool:
        ...
        for signal in entry_signals:
            ...
            if self.current_position is not None:
                ...
                if (
                    ...
                    and signal.signal_type == SignalType.ENTRY_LONG
                ) or (
                    self.current_position["direction"] == "SHORT"
                    and signal.signal_type == SignalType.ENTRY_SHORT
                ):
                    continue

                # If position is in the opposite direction, close it first
                signal_events.extend(
                    self._create_exit_events(
            ...
            direction = (
                "LONG" if signal.signal_type == SignalType.ENTRY_LONG else "SHORT"
            )
            ...
            if "take_profit" in signal.metadata and isinstance(
                signal.metadata["take_profit"], dict
            ):
    ...
    def _check_stop_loss(
        self, symbol: str, timestamp: pd.Timestamp, current_price: float
    ) -> List[SignalEvent]:
        ...
        if self.current_position["direction"] == "LONG":
            if current_price <= self.current_position["stop_loss"]:
                return self._create_exit_events(
                    "LONG", symbol, timestamp, "Stop loss triggered"
                )
        else:  # SHORT
            if current_price >= self.current_position["stop_loss"]:
                return self._create_exit_events(
                    "SHORT", symbol, timestamp, "Stop loss triggered"
                )
    ...
    def _check_take_profit(
        ...
        if (
            ...
            or self.current_position["take_profit"] is None
        ):
            return []
        ...
        if self.current_position["direction"] == "LONG":
            if current_price >= self.current_position["take_profit"]:
                return self._create_exit_events(
                    "LONG", symbol, timestamp, "Take profit triggered"
                )
        else:  # SHORT
            if current_price <= self.current_position["take_profit"]:
                return self._create_exit_events(
                    "SHORT", symbol, timestamp, "Take profit triggered"
                )

        return []

    def _create_exit_events(
        self, direction: str, symbol: str, timestamp: pd.Timestamp, reason: str
    ) -> List[SignalEvent]:
    ...
    def _update_stops(self, current_price: float, current_bar: pd.Series) -> None:
    ...
    def generate_signals(
        ...
        signal_events.extend(
            self._process_exit_signals(signals, symbol, timestamp, current_price)
        )

        # Then process entry signals
        signal_events.extend(
            self._process_entry_signals(
                signals, symbol, timestamp, current_price, portfolio
            )
        )
