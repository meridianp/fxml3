...
class FREDDataFeed(DataFeed):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not self.api_key:
            raise ValueError(
                "FRED API key not provided. Set config['api_key'] or FRED_API_KEY environment variable."
            )
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    raise Exception(
                        f"Failed to fetch data from FRED after {self.max_retries} attempts: {e}"
                    )
    ...
    def _apply_rate_limit(self):
        ...
        if self.api_calls >= self.api_call_limit:
            ...
            logger.warning(
                f"FRED API rate limit reached. Sleeping for {sleep_time:.2f} seconds."
            )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            series_info = self._make_api_request(
                SERIES_INFO_ENDPOINT, {"series_id": series_id}
            )
            ...
            metadata = {
                ...
                "frequency": series_info.get("seriess", [{}])[0].get(
                    "frequency_short", ""
                ),
                "seasonal_adjustment": series_info.get("seriess", [{}])[0].get(
                    "seasonal_adjustment_short", ""
                ),
                "last_updated": series_info.get("seriess", [{}])[0].get(
                    "last_updated", ""
                ),
            }
    ...
    def get_indicator_by_name(self, indicator_name: str, **kwargs: Any) -> pd.DataFrame:
        """
        ...
        if indicator_name not in COMMON_INDICATORS:
            ...
            raise ValueError(
                f"Unknown indicator: {indicator_name}. Valid indicators: {valid_indicators}"
            )
