...
class AlphaVantageNewsAPI:
    ...
    def get_market_sentiment(
        self, symbols: List[str], time_window_hours: int = 24
    ) -> Dict[str, Dict]:
    ...
    def _process_news_data(self, raw_data: Dict, symbol: str) -> Dict:
        ...
        for article in feed:
            ...
            if relevant_sentiments and article["title"] not in processed_articles:
                ...
                avg_sentiment = sum(s["sentiment"] for s in relevant_sentiments) / len(
                    relevant_sentiments
                )
                avg_relevance = sum(s["relevance"] for s in relevant_sentiments) / len(
                    relevant_sentiments
                )
        ...
        if sentiments:
            ...
            weighted_avg = (
                sum(weighted_sentiments) / sum(relevances) if sum(relevances) > 0 else 0
            )
...
def get_forex_news_sentiment(
    symbol: str, api_key: Optional[str] = None, hours_back: int = 24
) -> Dict:
    """Get news sentiment for a forex pair.
    ...
    return api.get_forex_sentiment(symbol=symbol, time_from=time_from, limit=100)


def analyze_market_sentiment(
    symbols: List[str], api_key: Optional[str] = None
) -> pd.DataFrame:
