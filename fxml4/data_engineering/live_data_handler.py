...
from fxml4.data_engineering.data_feeds.ib_feed import IBAPIApp, IBDataFeed
...
class LiveDataHandler:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.market_schedule = MARKET_SCHEDULES.get(
            self.market_type, MARKET_SCHEDULES["forex"]
        )
    ...
    def start(self):
        ...
        self.health_check_thread = threading.Thread(
            target=self._health_check_loop, daemon=True, name="IBHealthCheck"
        )
        self.health_check_thread.start()

        # Start data processing thread
        self.data_processing_thread = threading.Thread(
            target=self._data_processing_loop, daemon=True, name="DataProcessor"
        )
        ...
        while self.running:
            try:
                ...
                if (
                    ...
                    and self._is_market_trading_hours()
                ):
    ...
    def _attempt_connection(self):
        ...
        logger.info(
            f"Attempting to connect to IB (attempt {self.reconnect_attempts + 1}/{self.max_reconnect_attempts})"
        )
    ...
    def _resubscribe_all(self):
        ...
        if (
            not self.data_feed
            or not self.data_feed.app
            or not self.data_feed.app.connected
        ):
            return
    ...
    def _check_ib_connection(self) -> bool:
        ...
        if (
            not self.data_feed
            or not hasattr(self.data_feed, "app")
            or not self.data_feed.app
        ):
            return False
    ...
    def _handle_completed_candles(
        self, symbol: str, completed_candles: Dict[int, Dict[str, Any]]
    ):
        """
        ...
        for timeframe, candle in completed_candles.items():
            logger.debug(
                f"Completed {timeframe}m candle for {symbol}: "
    ...
    def _update_derived_timeframes(self, symbol: str, one_min_candle: Dict[str, Any]):
        ...
        try:
            ...
            if self.timeframe_converter:
                ...
                # Log update
                logger.debug(
                    f"Updated derived timeframes for {symbol} based on new 1-minute candle"
                )
        except Exception as e:
    ...
    def _check_and_trigger_callbacks(self):
        ...
        for symbol, callbacks in list(self.data_callbacks.items()):
            for timeframe, callback_list in list(callbacks.items()):
                ...
                try:
                    if timeframe == "1m":
                        candles = self.tick_aggregator.get_candles(
                            symbol, timeframe=1, limit=100
                        )
                    else:
                        # Try to get from timeframe converter if available
                        if self.timeframe_converter:
                            candles = self.timeframe_converter.get_data(
                                symbol, timeframe
                            )
                        else:
                            ...
                            if timeframe.endswith("d"):
                                minutes *= 1440
                            candles = self.tick_aggregator.get_candles(
                                symbol, timeframe=minutes, limit=100
                            )
                    ...
                    if not candles.empty and callback_list:
                        ...
                        for callback in callback_list:
                            try:
                                ...
                                logger.error(
                                    f"Error in callback for {symbol} {timeframe}: {e}"
                                )
    ...
    def get_latest_candles(
        ...
        try:
            ...
            if timeframe == "1m":
                return self.tick_aggregator.get_candles(
                    symbol, timeframe=1, limit=limit
                )
            ...
            return self.tick_aggregator.get_candles(
                symbol, timeframe=minutes, limit=limit
            )
        ...
        with self._lock:
            try:
                ...
                if (
                    ...
                    and timeframe in self.data_callbacks[symbol]
                    and callback in self.data_callbacks[symbol][timeframe]
                ):
    ...
    def get_historical_data(
        ...
        if not self.data_feed or self.connection_state != ConnectionState.CONNECTED:
            logger.error(
                "Cannot get historical data: not connected to Interactive Brokers"
            )
            return pd.DataFrame()
