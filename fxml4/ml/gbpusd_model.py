...
class GBPUSDModel:
    ...
    def _set_default_params(self):
        ...
        if self.model_type == "logistic":
            ...
            defaults = {
                ...
                "objective": (
                    "multi:softmax" if self.n_classes > 2 else "binary:logistic"
                ),
    ...
    def prepare_features(
        ...
        features = self._validate_and_prepare_data(data)
        ...
        features = self._create_technical_features(features, add_enhanced_features)

        # Add custom 4h timeframe features
        features = self._add_custom_features(features)

        # Add pivot point features
        if add_pivot_points:
            features = self._add_pivot_point_features(
                features, target_horizon, add_enhanced_features
            )

        # Add confluence indicators
        if add_enhanced_features:
            features = self._add_confluence_indicators(features)
        ...
        if create_target:
            features = self._create_target_variable(
                features, target_horizon, target_threshold, labeling_method
            )
        ...
        logger.info(
            f"Prepared {len(features)} rows with {len(features.columns)} features"
        )
        return features
    ...
    def _add_session_features(self, features: pd.DataFrame) -> pd.DataFrame:
    ...
    def _create_technical_features(
        self, features: pd.DataFrame, add_enhanced_features: bool
    ) -> pd.DataFrame:
        """Create technical indicators using FXML2 parameters."""
        return create_technical_features(
            features,
            indicators=[
                "sma",
                "ema",
    ...
    def _add_custom_features(self, features: pd.DataFrame) -> pd.DataFrame:
        """Add custom features for 4h timeframe."""
        # Daily range as percentage of price
        features["daily_range_pct"] = (
            (features["high"].rolling(6).max() - features["low"].rolling(6).min())
            / features["close"]
            * 100
        )
        ...
        if "volatility_14" not in features.columns:
            features["volatility_14"] = (
                features["close"].pct_change().rolling(14).std() * 100
            )

        # Weekly open/close change - from FXML2
        if "weekly_change" not in features.columns:
            features["weekly_change"] = (
                (features["close"] - features["close"].shift(42))
                / features["close"].shift(42)
                * 100
            )

        return features

    def _add_pivot_point_features(
        self, features: pd.DataFrame, target_horizon: int, add_enhanced_features: bool
    ) -> pd.DataFrame:
        """Add pivot point analysis features."""
        logger.info("Adding pivot point analysis features")
        try:
            # Add weekly and session pivot points
            features = calculate_weekly_pivot_points(features)
            features = calculate_session_pivot_levels(features)

            # Add pivot effectiveness features
            features = self._add_pivot_effectiveness_features(features, target_horizon)

            # Add enhanced pivot-based features if requested
            if add_enhanced_features:
                features = self._add_enhanced_pivot_features(features)
    ...
    def _add_pivot_effectiveness_features(
        self, features: pd.DataFrame, window: int
    ) -> pd.DataFrame:
        """Add pivot point effectiveness features."""
        ...
        for col in ["R1", "R2", "S1", "S2"]:
            if col in features.columns:
                if col.startswith("R"):  # Resistance level
                    features[f"hit_{col}_{window}"] = (
                        features["high"].rolling(window).max() >= features[col]
                    )
                else:  # Support level
                    features[f"hit_{col}_{window}"] = (
                        features["low"].rolling(window).min() <= features[col]
                    )
        return features

    def _add_enhanced_pivot_features(self, features: pd.DataFrame) -> pd.DataFrame:
        """Add enhanced pivot-based breakout features."""
        pivot_cols = [
            col
            ...
            if "r1" in col.lower():
                ...
                if f"{prefix}_r1" in features.columns:
                    features[f"{prefix}_breakout_up"] = (
                        features["close"] > features[f"{prefix}_r1"]
                    ).astype(int)
                ...
                if f"{prefix}_s1" in features.columns:
                    features[f"{prefix}_breakout_down"] = (
                        features["close"] < features[f"{prefix}_s1"]
                    ).astype(int)

        return features

    def _add_confluence_indicators(self, features: pd.DataFrame) -> pd.DataFrame:
        """Add enhanced confluence indicators from FXML2."""
        logger.info("Adding enhanced confluence indicators")
        try:
            # Ensure required components exist
            required_cols = ["bb_squeeze", "macd_cross_up", "close"]
    ...
    def _add_lagged_features(self, features: pd.DataFrame) -> pd.DataFrame:
        """Add lagged features using FXML2 parameters."""
        ...
        pivot_distance_cols = [
            col for col in features.columns if col.startswith("distance_to_")
        ]
        session_distance_cols = [
            col for col in features.columns if col.startswith("dist_")
        ]
    ...
    def _create_target_variable(
        ...
        """Create target variable using specified labeling method."""
        if labeling_method == "fixed_threshold":
            return self._create_fixed_threshold_target(
                features, target_horizon, target_threshold
            )
        elif labeling_method == "volatility_adjusted":
            return self._create_volatility_adjusted_target(
                features, target_horizon, target_threshold
            )
        elif labeling_method == "trend_adjusted":
            return self._create_trend_adjusted_target(
                features, target_horizon, target_threshold
            )
        else:
            logger.warning(
                f"Unknown labeling method '{labeling_method}', using fixed threshold"
            )
            return self._create_fixed_threshold_target(
                features, target_horizon, target_threshold
            )

    def _create_fixed_threshold_target(
        self, features: pd.DataFrame, target_horizon: int, target_threshold: float
    ) -> pd.DataFrame:
        """Create target using fixed threshold labeling."""
    ...
    def _create_volatility_adjusted_target(
        self, features: pd.DataFrame, target_horizon: int, target_threshold: float
    ) -> pd.DataFrame:
        """Create target using volatility-adjusted thresholds."""
    ...
    def _create_trend_adjusted_target(
        self, features: pd.DataFrame, target_horizon: int, target_threshold: float
    ) -> pd.DataFrame:
        """Create target using trend-adjusted thresholds."""
        ...
        down_thresh = (
            -target_threshold + trend_adjustment
        )  # Higher threshold in uptrend
    ...
    def train(
        ...
        if use_cv:
            ...
            cv_scores = cross_val_score(
                self.model, X_scaled, y, cv=cv, scoring="f1_weighted"
            )
...
def train_gbpusd_model(
    ...
    logger.info(
        f"Prepared features with {len(features)} rows and {len(features.columns)} columns"
    )
...
def load_and_evaluate_model(
    model_name: str, data_path: str, target_horizon: int = 12, directory: str = "models"
) -> Dict[str, Any]:
