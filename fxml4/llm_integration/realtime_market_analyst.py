...
class RealtimeMarketAnalyst:
    ...
    async def analyze_market_conditions(
        self, symbol: str, price_data: Dict[str, Any], timeframe: str = "4h"
    ) -> Dict[str, Any]:
        ...
        for component in ["market_structure", "sentiment", "technical_patterns"]:
            if (
                market_analysis[component]
                and "confidence" in market_analysis[component]
            ):
                confidence_scores.append(market_analysis[component]["confidence"])

        market_analysis["overall_confidence"] = (
    ...
    async def _analyze_sentiment(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if sentiment_data["articles"]:
                recent_headlines = [
                    article["title"] for article in sentiment_data["articles"][:5]
                ]
...
5. confidence: 0-1 score"""
                ...
                try:
                    ...
                    llm_sentiment["quantitative_score"] = sentiment_data[
                        "sentiment_score"
                    ]
    ...
    async def _analyze_technical_patterns(
        ...
        try:
            ...
            rag_results = self.rag_system.query(
                f"Technical patterns similar to: {price_movement}", k=3
            )
...
5. confidence: overall confidence score"""
            ...
            try:
                ...
                return {"patterns": ["Unable to parse LLM response"], "confidence": 0.4}
...
5. confidence: 0-1 score"""
    ...
    async def _generate_trade_recommendation(
        self, symbol: str, price_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            current_price = (
                price_data.get("close", [])[-1] if price_data.get("close") else 0
            )
            ...
            try:
                ...
                if recommendation.get("action") in ["buy", "sell"]:
                    ...
                    if recommendation.get("stop_loss"):
                        ...
                        tp_distance = abs(
                            recommendation.get("take_profit", current_price)
                            - current_price
                        )

                        # Ensure minimum 1:1 risk-reward
                        if tp_distance < sl_distance:
                            recommendation["take_profit"] = current_price + (
                                2
                                * sl_distance
