...
with patch.dict(
    ...
    from fxml4.brokers.adapters.base import OrderStatus


class RiskCheckResult:
    """Risk check result for testing purposes."""
    ...
    def __init__(
        ...
        max_allowed_quantity: float = None,
    ):
        self.passed = passed
        self.message = message
        self.risk_factors = risk_factors or []
        self.max_allowed_quantity = max_allowed_quantity
...
class TestOrderModels:
    ...
    def test_order_data_with_optional_fields(self):
        """Test OrderData with optional fields."""
...
class TestMockOrderManagementService:
    ...
    @pytest.mark.asyncio
    async def test_create_market_order(self, service):
    ...
    @pytest.mark.asyncio
    async def test_create_stop_order(self, service):
        ...
        assert order.stop_price == 150.50
        assert order.time_in_force == TimeInForce.FOK
        assert order.limit_price is None  # Stop orders don't have limit price

    @pytest.mark.asyncio
    async def test_create_stop_limit_order(self, service):
        ...
        assert order.limit_price == 0.9100
        assert order.stop_price == 0.9110
        assert order.time_in_force == TimeInForce.IOC

    @pytest.mark.asyncio
    async def test_create_order_from_buy_signal(self, service):
    ...
    async def test_create_order_from_sell_signal(self, service):
        ...
        assert order.metadata["signal_confidence"] == 0.7
    ...
    async def test_execute_order(self, service):
        ...
        assert execution.exchange_order_id.startswith("EX")
    ...
    @pytest.mark.asyncio
    async def test_execute_already_filled_order(self, service):
        """Test execution of already filled order."""
    ...
    @pytest.mark.asyncio
    async def test_cancel_pending_order(self, service):
    ...
    @pytest.mark.asyncio
    async def test_cancel_filled_order(self, service):
        """Test cancellation of filled order."""
        # Create and execute an order
    ...
    async def test_cancel_nonexistent_order(self, service):
        ...
        with pytest.raises(ValueError, match="Order .* not found"):
            await service.cancel_order("nonexistent_order")

    @pytest.mark.asyncio
    async def test_get_all_orders(self, service):
        ...
        order3 = await service.create_order("USDJPY", OrderSide.BUY, 3000.0)
        ...
        assert order_ids == [order3.id, order2.id, order1.id]

    @pytest.mark.asyncio
    async def test_get_orders_by_symbol(self, service):
        """Test filtering orders by symbol."""
        # Create test orders
        await service.create_order("EURUSD", OrderSide.BUY, 1000.0)
        await service.create_order("GBPUSD", OrderSide.SELL, 2000.0)
        await service.create_order("EURUSD", OrderSide.BUY, 3000.0)
        ...
        gbpusd_orders = await service.get_orders(symbol="GBPUSD")
        assert len(gbpusd_orders) == 1
        assert gbpusd_orders[0].symbol == "GBPUSD"

    @pytest.mark.asyncio
    async def test_get_orders_by_status(self, service):
        ...
        # Execute one order and cancel another
        ...
        cancelled_orders = await service.get_orders(status=OrderStatus.CANCELLED)
        assert len(cancelled_orders) == 1
        assert cancelled_orders[0].id == order3.id
    ...
    async def test_get_orders_pagination(self, service):
        ...
        for i in range(5):
            await service.create_order(f"PAIR{i}", OrderSide.BUY, 1000.0 * (i + 1))
        ...
        page2_orders = await service.get_orders(limit=2, offset=3)
        assert len(page2_orders) == 2

    @pytest.mark.asyncio
    async def test_get_executions(self, service):
        """Test getting executions for an order."""
        # Create and execute an order
        ...
        assert executions[0].quantity == 5000.0
    ...
    @pytest.mark.asyncio
    async def test_risk_checks_pass(self, service):
    ...
    async def test_order_callback_notifications(self, service):
        ...
        def test_callback(order):
            callback_calls.append((order.id, order.status))
        ...
        assert callback_calls[0] == (order.id, OrderStatus.PENDING)

        # Execute order - should trigger callback again
        await service.execute_order(order.id)
        assert len(callback_calls) == 2
        assert callback_calls[1] == (order.id, OrderStatus.FILLED)
    ...
    async def test_execution_callback_notifications(self, service):
        ...
        def test_callback(execution):
            execution_calls.append((execution.order_id, execution.quantity))
        ...
        assert execution_calls[0] == (order.id, 2000.0)
