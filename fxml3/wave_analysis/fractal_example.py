...
from fxml3.visualization.chart import plot_wave_analysis, plot_interactive_chart
...
def create_synthetic_multi_timeframe_data() -> Dict[str, pd.DataFrame]:
    ...
    daily_df = create_test_data(
        ...
        base_price=100.0,
        volatility=1.0,
        wave_pcts=[0.15, -0.08, 0.25, -0.1, 0.18, -0.12, 0.09, -0.05, 0.12]  # Complex pattern with sub-waves
    )
...
def create_test_data(
    ...
    for i in range(total_waves):
        ...
        if i == total_waves - 1:
            ...
            if (pattern_type == "impulse" and i == 2) or (pattern_type == "corrective" and i == 0):
...
def analyze_multi_timeframe_data(
    ...
    for timeframe, result in results.items():
        ...
        corrective_cols = [col for col in labeled_df.columns if col.startswith("corrective_")]
        ...
        for col in impulse_cols + corrective_cols:
            if col in labeled_df.columns:
                ...
                if len(waves) > 0:
                    ...
                    chart_title = f"{degree} Degree {pattern_type} Pattern - {timeframe}"
...
def print_structure(
    ...
    for wave_key, wave in structure.items():
        ...
        # Print wave details
        print(f"{indent_str}[{degree}] {wave_type.capitalize()} Wave {wave_num} ({timeframe}): {start} → {end}")
...
def main():
    ...
    # Run analysis
    print(f"Analyzing multi-timeframe data ({sum(len(df) for df in data_dict.values())} total rows)...")
