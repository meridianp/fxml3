...
from fxml3.backtesting.rl_training import train_agent, evaluate_agent
...
class PolicyOptimizer:
    ...
    def _train_and_evaluate(self, config: Dict[str, Any]) -> Dict[str, float]:
        ...
        train_metrics = train_agent(
            ...
            checkpoint_dir=os.path.join(config.get("checkpoint_dir", "./checkpoints"), str(tune.get_trial_id())),
            early_stopping_patience=config.get("early_stopping_patience", 20),
            tensorboard_dir=os.path.join(config.get("tensorboard_dir", "./logs"), str(tune.get_trial_id()))
        )
    ...
    def run_grid_search(self, experiment_name: str = "ppo_grid_search") -> Dict[str, Any]:
        ...
        reporter = CLIReporter(
            metric_columns=[self.optimization_metric, "episode_reward_mean", "training_loss"],
        ...
        print(f"Best trial final {self.optimization_metric}: {best_trial.last_result[self.optimization_metric]}")
        
        # Save best configuration
        best_config = best_trial.config
        os.makedirs("./optimization_results", exist_ok=True)
        with open(f"./optimization_results/{experiment_name}_best_config.json", "w") as f:
    ...
    def visualize_optimization_results(self, top_n: int = 5, save_path: Optional[str] = None):
        ...
        for i, (idx, row) in enumerate(top_trials.iterrows()):
            ...
            print(f"  Network: {row['config/hidden_layers']}, {row['config/activation']}")
...
def detect_market_regime(state: np.ndarray, volatility_threshold: float = 1.5, trend_threshold: float = 0.6) -> str:
    """
...
if __name__ == "__main__":
    ...
    # Visualize results
    optimizer.visualize_optimization_results(top_n=5, save_path="./optimization_results/top_configs.png")