...
class PerformanceMetrics:
    ...
    def calculate_risk_reward_metrics(
        ...
        for outcome in actual_outcomes:
            # Risk/reward ratio
            if "risk_reward_ratio" in outcome and outcome["risk_reward_ratio"] != float('inf'):
            ...
            # Time to target
            if outcome.get("correct", False) and outcome.get("time_to_target") is not None:
    ...
    def calculate_multi_timeframe_metrics(
        ...
        for tf, results in multi_tf_results.items():
            ...
            metrics["overall"]["total_predictions"] += tf_metrics["overall"]["predictions_made"]
            metrics["overall"]["correct_predictions"] += tf_metrics["overall"]["correct_predictions"]
    ...
    def calculate_profitability(
        ...
        if total_profit > 0:
            metrics["cost_metrics"]["cost_as_pct_of_profit"] = (total_cost / total_profit) * 100
        
        # Add advanced metrics
        metrics["expectancy"] = 0.0
        if (metrics["win_count"] + metrics["loss_count"]) > 0:
            win_rate = metrics["win_count"] / (metrics["win_count"] + metrics["loss_count"])
        ...
        if metrics["avg_loss_pct"] > 0:
            win_rate = metrics["win_count"] / (metrics["win_count"] + metrics["loss_count"]) if (metrics["win_count"] + metrics["loss_count"]) > 0 else 0
            avg_win_loss_ratio = metrics["avg_win_pct"] / metrics["avg_loss_pct"]
            metrics["kelly_percentage"] = win_rate - ((1 - win_rate) / avg_win_loss_ratio)
            metrics["kelly_percentage"] = max(0, metrics["kelly_percentage"]) * 100  # Keep positive only, convert to percentage
    ...
    def calculate_market_impact_metrics(
        ...
        for i, outcome in enumerate(actual_outcomes):
            ...
            if "spread_cost" in outcome:
                ...
                if "execution_price" in outcome and outcome["execution_price"] > 0:
                    spread_pips = (outcome["spread_cost"] / outcome["execution_price"]) * 10000
                else:
            ...
            if "total_cost" in outcome and "execution_price" in outcome and outcome["execution_price"] > 0:
                position_size = outcome.get("position_size", 10000.0)
        ...
        if slippage_values and spread_values:
            ...
            norm_slippage = np.array(slippage_values) / max(5.0, np.max(slippage_values))
