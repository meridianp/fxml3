...
from fxml3.strategy.entry_signals import SignalType, EntrySignal
...
class RiskManager:
    ...
    def calculate_dynamic_stop_loss(
        ...
        adjusted_risk = self.max_risk_per_trade * volatility_factor  # Adjust risk based on volatility
        adjusted_risk = min(adjusted_risk, self.max_risk_per_trade * 1.5)  # Cap at 150% of max risk
        adjusted_risk = max(adjusted_risk, self.max_risk_per_trade * 0.5)  # Floor at 50% of max risk
    ...
    def _apply_volatility_adjustment(
        ...
        if atr is None:
            ...
            return stop_loss, {"volatility_adjustment_applied": False, "reason": "insufficient_data"}
        ...
        if len(data) >= 50:  # Make sure we have enough data for historical comparison
            recent_atr = atr
            historical_atr = self._calculate_historical_atr(data, current_idx, window=50)
        ...
        return stop_loss, {"volatility_adjustment_applied": False, "reason": "no_adjustment_needed"}
            
    def _calculate_atr(self, data: pd.DataFrame, current_idx: int, window: int = 14) -> Optional[float]:
        """
    ...
    def _calculate_historical_atr(self, data: pd.DataFrame, current_idx: int, window: int = 50) -> float:
        """
        ...
        if current_idx < window * 2:
            ...
            if current_idx < window:
                return self._calculate_atr(data, current_idx, max(5, current_idx // 2)) or 0.0
            else:
                historical_slice = data.iloc[0:current_idx-window]
        else:
            # Use window data from further back
            historical_slice = data.iloc[current_idx-window*2:current_idx-window]
    ...
    def _validate_stop_with_multiple_timeframes(
        ...
        if entry_signal.timeframe in ordered_timeframes:
            ...
            higher_timeframes = ordered_timeframes[current_tf_idx+1:] if current_tf_idx < len(ordered_timeframes) - 1 else []
            lower_timeframes = ordered_timeframes[:current_tf_idx] if current_tf_idx > 0 else []
            ...
            if multi_tf_data and higher_timeframes:
                adjusted_stop, validation_meta = self._adjust_stop_with_higher_timeframes(
                    stop_loss, 
                    entry_signal, 
                    higher_timeframes, 
                    multi_tf_data
        ...
        for tf in higher_timeframes:
            ...
            if signal_type == SignalType.LONG:
                ...
                for level in key_levels:
                    if level["type"] == "support" and stop_loss > level["price"] > stop_loss * 0.99:
                ...
                for level in key_levels:
                    if level["type"] == "resistance" and stop_loss < level["price"] < stop_loss * 1.01:
    ...
    def _calculate_corrective_wave_stop(
        ...
        if pattern == "abc_completion":
            ...
            if signal_type == SignalType.LONG:
                ...
                recent_low = data.iloc[current_idx-lookback:current_idx+1]['low'].min()
                ...
                lookback = min(15, current_idx)
                recent_high = data.iloc[current_idx-lookback:current_idx+1]['high'].max()
    ...
    def detect_pattern_invalidation(
        ...
        if self._is_impulse_pattern(pattern_type):
            return self._check_impulse_invalidation(data, pattern_start_idx, current_idx)
        elif self._is_corrective_pattern(pattern_type):
            return self._check_corrective_invalidation(data, pattern_start_idx, current_idx)
        else:
    ...
    def _check_impulse_invalidation(
        ...
        for i in range(pattern_start_idx, current_idx + 1):
            ...
            if any(row.get(f'wave_1_{point}', False) for point in ['end', 'high', 'complete']):
                wave1_high = row['high']
                wave1_low = row['low']
                
            # Check for wave 4 markers
            if any(row.get(f'wave_4_{point}', False) for point in ['end', 'low', 'complete']):
                wave4_high = row['high']
                wave4_low = row['low']
        
        # If we found both wave points, check for overlap
        if (wave1_high is not None and wave1_low is not None and 
            wave4_high is not None and wave4_low is not None):
    ...
    def _is_impulse_pattern(self, pattern: str) -> bool:
        ...
        impulse_patterns = [
            "wave_3_start", "wave_5_start", "impulse_complete",
            "wave_1_complete", "wave_3_complete", "wave_5_complete"
        ]
    ...
    def _is_corrective_pattern(self, pattern: str) -> bool:
        ...
        corrective_patterns = [
            "abc_completion", "correction_complete", "triangle_breakout",
            "flat_pattern", "zigzag_complete", "double_correction"
        ]
    ...
    def _find_wave4_low(self, data: pd.DataFrame, index: int) -> Optional[float]:
        ...
        for i in range(index, max(0, index-15), -1):
            ...
            wave4_indicators = [
                'wave_4_end', 'impulse_wave_4_end', 
                'fourth_wave_end', 'wave_4_complete'
            ]
