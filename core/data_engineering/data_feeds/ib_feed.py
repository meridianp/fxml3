    ...
    def tickPrice(self, reqId: int, tickType: int, price: float, attrib):
        ...
        if reqId in self.symbol_map:
            ...
            if self.tick_count % 1000 == 0:
                logger.info(
                    f"Received {self.tick_count} ticks, latest symbol: {symbol}"
                )
...
class IBDataFeed(DataFeed):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not IB_API_AVAILABLE:
            raise ImportError(
                "Interactive Brokers API not available. Please install ibapi package."
            )
        ...
        logger.info(
            f"Initialized IB data feed with host={self.host}, port={self.port}, real_time={self.real_time_updates}"
        )
    ...
    def fetch_data(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            logger.info(
                f"Requesting {ib_bar_size} data for {symbol} with duration {duration_str}"
            )
            ...
            try:
                ...
                if (
                    ...
                    and "US/Eastern" in df["timestamp"].iloc[0]
                ):
                    # Remove timezone info for now
                    df["timestamp"] = df["timestamp"].apply(
                        lambda x: x.split(" US/Eastern")[0]
                    )
    ...
    def subscribe_market_data(self, symbol: str, snapshot: bool = False) -> int:
        ...
        logger.info(
            f"Subscribed to {'snapshot' if snapshot else 'streaming'} market data for {symbol} (reqId: {req_id})"
        )
    ...
    def cancel_market_data(self, symbol: str) -> bool:
        ...
        if symbol in self.market_data_subscriptions:
            ...
            if self.app and self.app.connected:
                self.app.cancelMktData(req_id)
                logger.info(
                    f"Canceled market data subscription for {symbol} (reqId: {req_id})"
                )
    ...
    def start_tick_processor(self):
        ...
        self.tick_processor_thread = threading.Thread(
            target=self._tick_processor_loop, daemon=True
        )
                    ...
                    if tick["type"] == "price" and tick["price"] is not None:
                        ...
                        if self.tick_aggregator:
                            ...
                            if completed_candles:
                                self._handle_completed_candles(
                                    symbol, completed_candles
                                )
        ...
        for timeframe, candle in completed_candles.items():
            ...
            try:
                ...
                if timeframe == 1:
                    ...
                    if result:
                        logger.debug(
                            f"Stored {timeframe}m candle for {symbol} in TimescaleDB"
                        )
                    else:
                        logger.warning(
                            f"Failed to store {timeframe}m candle for {symbol} in TimescaleDB"
                        )
            except Exception as e:
    ...
    def get_realtime_candles(
        self, symbol: str, timeframe: Union[int, str] = 1, limit: int = 100
    ) -> pd.DataFrame:
        ...
        derived_timeframes = (
            ["5m", "15m", "30m", "1h", "4h", "1d"] if self.timeframe_converter else []
        )
    ...
    def get_latest_tick(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        result = {
            "symbol": symbol,
            "timestamp": self.app.last_tick_time.get(
