...
class ElliottWaveCount:
    ...
    def to_dict(self) -> Dict:
        ...
        """
        return {"waves": [wave.to_dict() for wave in self.waves]}
    ...
    def from_dict(cls, data: Dict) -> "ElliottWaveCount":
        ...
        """
        waves = [
            ElliottWavePattern.from_dict(wave_data)
...
class ElliottWaveAnalyzer:
    ...
    def detect_peaks_and_troughs(
        ...
        for i in range(len(result_df)):
            ...
            result_df.at[idx, "is_peak"] = is_peak(
                result_df[high_col].values, i, window
            )
            result_df.at[idx, "is_trough"] = is_trough(
    ...
    def compute_waves(
        self, df: pd.DataFrame, min_wave_size_pct: Optional[float] = None
    ) -> List[Dict]:
    ...
    def validate_fibonacci_relationship(
        self, ratio: float, expected_ratio: float, tolerance: Optional[float] = None
    ) -> bool:
        ...
        """
        return self.fibonacci_calculator.validate_ratio(
            ...
            for i, wave in enumerate(pattern["waves"]):
                subwave = ElliottWavePattern(
                    ...
                    position=(
                        WavePosition.START
                        if i == 0
                        else WavePosition.MIDDLE if i < 4 else WavePosition.END
                    ),
                )
        ...
        for pattern in corrective_patterns:
            ...
            for i, wave in enumerate(pattern["waves"]):
                subwave = ElliottWavePattern(
                    wave_type=(
                        WaveType.ZIGZAG
                        if pattern["pattern_subtype"].startswith("zigzag")
                        else WaveType.FLAT
                    ),
                    start_idx=wave["start_idx"],
                    end_idx=wave["end_idx"],
                    ...
                    position=(
                        WavePosition.START
                        if i == 0
                        else WavePosition.MIDDLE if i == 1 else WavePosition.END
                    ),
                )
                subwaves.append(subwave)
