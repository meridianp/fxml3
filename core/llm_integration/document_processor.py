...
try:
    ...
    logger.warning(
        "PyPDF2 or langchain not available. Document processing will be limited."
    )
...
def create_chunks(
    ...
    try:
        ...
        logger.warning(
            f"Error using langchain text splitter: {str(e)}. Falling back to simple chunking."
        )
        return _simple_text_chunks(text, chunk_size, overlap, filter_empty)
...
def process_directory(
    ...
    if recursive:
        ...
        file_paths = [
            ...
            and f.lower().endswith((".pdf", ".txt", ".md"))
        ]
