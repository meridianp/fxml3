...
class EconomicFeatureEngineer:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.regime_detection_method = self.economic_config.get(
            ...
            get_config().get("ml.economic_features.regime_detection_method", "kmeans"),
        )
        self.n_regimes = self.economic_config.get(
            "n_regimes", get_config().get("ml.economic_features.n_regimes", 3)
        )
        self.use_derived_features = self.economic_config.get(
            "use_derived_features",
            get_config().get("ml.economic_features.use_derived_features", True),
        )

        # Load indicator weights (importance for regime detection)
        self.indicator_weights = self.economic_config.get(
            "indicator_weights",
            get_config().get("ml.economic_features.indicator_weights", {}),
        )
        ...
        self.lookback_periods = self.economic_config.get(
            "lookback_periods",
            get_config().get("ml.economic_features.lookback_periods", [3, 6, 12]),
        )
    ...
    def create_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.use_derived_features:
            ...
            available_indicators = [
                ind for ind in growth_indicators if ind in df.columns
            ]
            ...
            available_indicators = [
                ind for ind in inflation_indicators if ind in df.columns
            ]
            ...
            if "UNRATE" in df.columns:
                df["labor_market_health"] = (
                    -1 * df["UNRATE_zscore"]
                )  # Invert so positive is healthy
    ...
    def detect_economic_regime(self, data: pd.DataFrame) -> pd.Series:
        ...
        if self.regime_detection_method == "kmeans":
            ...
            logger.warning(
                f"Unknown regime detection method: {self.regime_detection_method}"
            )
