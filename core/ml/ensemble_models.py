    ...
    def _get_meta_features(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.use_probabilities:
            meta_features = np.zeros(
                (X.shape[0], len(self.fitted_base_models_) * n_classes)
            )
...
class BlendingEnsemble(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "BlendingEnsemble":
        ...
        if self.use_probabilities:
            blend_features = np.zeros(
                (X_blend.shape[0], len(self.base_models) * n_classes)
            )
        else:
...
class DynamicEnsemble(BaseEstimator, ClassifierMixin):
    ...
    def update_performance(
        self, y_true: np.ndarray, y_pred: Dict[str, np.ndarray]
    ) -> None:
        ...
        for name in self.performance_history_:
            if name in y_pred:
                ...
                if len(self.performance_history_[name]) > self.window_size:
                    self.performance_history_[name] = self.performance_history_[name][
                        -self.window_size :
                    ]
