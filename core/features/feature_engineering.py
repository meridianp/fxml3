...
class UnifiedFeatureEngineer:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.basic_indicators = self.config.get(
            "basic_indicators",
    ...
    def _add_basic_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "bollinger" in self.basic_indicators:
            ...
            ratio = (
                (df["bb_width"] / (df["bb_middle"] + 1e-10))
                .rolling(squeeze_window)
                .mean()
            )
        ...
        df["weekly_change"] = (
            (df["close"] - df["close"].shift(5 * 24))
            / (df["close"].shift(5 * 24) + 1e-10)
            * 100
        )
    ...
    def _add_market_microstructure_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["close_to_high"] = (df["close"] - df["low"]) / (
    ...
    def _add_pattern_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Channel indicators
        df["channel_position"] = (df["close"] - df["support_20"]) / (
            df["resistance_20"] - df["support_20"] + 1e-10
        )

        # Trend strength (requires ATR)
        if "atr_14" in df.columns:
            df["trend_strength"] = (df["close"] - df["close"].shift(20)) / (
                df["atr_14"].rolling(window=20).mean() + 1e-10
            )
        else:
            df["trend_strength"] = (df["close"] - df["close"].shift(20)) / (
                df["close"].shift(20) + 1e-10
            )

        return df

    def _add_elliott_wave_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """Add Elliott Wave features (simplified version) - FIXED: No look-ahead bias.

        This method now ensures that Elliott Wave features are calculated using only
        historical data available at each point in time. No future data is used.
        """
        if not self.elliott_wave_features:
            return df

        # Initialize features - use forward fill approach to avoid leakage
        window_size = 100
        df["wave_trend"] = 0.0
        df["fib_support"] = df["close"]
        df["fib_resistance"] = df["close"]

        # CRITICAL FIX: Calculate features for each row using only past data
        for i in range(window_size, len(df)):
            # Use ONLY historical data: from max(0, i-window_size) to i-1 (excluding current point)
            # This ensures no look-ahead bias - we don't include the current data point
            window_data = df.iloc[
                max(0, i - window_size) : i
            ]  # Fixed: Changed from i to i (excluding current)

            if len(window_data) < 10:  # Skip if window too small
                continue

            # Find high and low in the HISTORICAL window only
            high_idx = window_data["high"].idxmax()
            low_idx = window_data["low"].idxmin()

            # Simple trend detection using only historical data
            if pd.notna(high_idx) and pd.notna(low_idx):
                # Calculate trend and Fibonacci levels for current point (index i)
                current_idx = df.index[i]

                if high_idx > low_idx:  # Uptrend in historical data
                    df.loc[current_idx, "wave_trend"] = 1.0
                    # Calculate Fibonacci levels based on historical swing
                    swing_range = (
                        window_data.loc[high_idx, "high"]
                        - window_data.loc[low_idx, "low"]
                    )
                    df.loc[current_idx, "fib_support"] = (
                        window_data.loc[low_idx, "low"] + 0.618 * swing_range
                    )
                    df.loc[current_idx, "fib_resistance"] = (
                        window_data.loc[low_idx, "low"] + 1.618 * swing_range
                    )
                else:  # Downtrend in historical data
                    df.loc[current_idx, "wave_trend"] = -1.0
                    swing_range = (
                        window_data.loc[high_idx, "high"]
                        - window_data.loc[low_idx, "low"]
                    ...
                    df.loc[current_idx, "fib_support"] = (
                        window_data.loc[high_idx, "high"] - 1.618 * swing_range
                    )
                    df.loc[current_idx, "fib_resistance"] = (
                        window_data.loc[high_idx, "high"] - 0.618 * swing_range
                    )

        # Forward fill the wave features to handle any remaining NaN values
        # Use pandas recommended methods instead of deprecated fillna(method=)
        df["wave_trend"] = df["wave_trend"].ffill().fillna(0)
        df["fib_support"] = df["fib_support"].ffill().fillna(df["close"])
        df["fib_resistance"] = df["fib_resistance"].ffill().fillna(df["close"])

        # Distance to Fibonacci levels (safe calculation)
        df["dist_to_fib_support"] = (df["close"] - df["fib_support"]) / (
            df["close"] + 1e-10
        )
        df["dist_to_fib_resistance"] = (df["fib_resistance"] - df["close"]) / (
            df["close"] + 1e-10
        )
    ...
    def _add_regime_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df["vol_regime"] = pd.qcut(
                vol_rolling, q=3, labels=[0, 1, 2], duplicates="drop"
            )
            df["vol_regime"] = pd.to_numeric(df["vol_regime"], errors="coerce").fillna(
                1
            )
        except:
            df["vol_regime"] = 1
        ...
        try:
            ...
            df["momentum_regime"] = pd.qcut(
                rsi_rolling, q=3, labels=[0, 1, 2], duplicates="drop"
            )
            df["momentum_regime"] = pd.to_numeric(
                df["momentum_regime"], errors="coerce"
            ).fillna(1)
        except:
    ...
    def _add_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in self.ma_periods:
            if f"sma_{period}" in df.columns:
                df[f"price_above_sma_{period}"] = (
                    df["close"] > df[f"sma_{period}"]
                ).astype(int)
                df[f"price_to_sma_{period}"] = df["close"] / (
                    df[f"sma_{period}"] + 1e-10
                )
    ...
    def _clean_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """Clean up features by handling NaN and inf values - FIXED: Updated deprecated methods."""
        # Replace infinities
        df = df.replace([np.inf, -np.inf], np.nan)

        # Forward fill then backward fill using recommended pandas methods
        df = df.ffill().bfill()
