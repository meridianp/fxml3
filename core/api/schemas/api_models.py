...
class DataRequest(BaseModel):
    ...
    start_date: Optional[str] = Field(
        None, description="Start date in ISO format (e.g., '2023-01-01')"
    )
    end_date: Optional[str] = Field(
        None, description="End date in ISO format (e.g., '2023-12-31')"
    )
    limit: Optional[int] = Field(
        None, description="Maximum number of data points to return"
    )
...
class BacktestResponse(BaseModel):
    ...
    report_url: Optional[str] = Field(
        None, description="URL to access the performance report"
    )
