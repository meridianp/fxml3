...
class SignalCombiner:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.method = self.config.get(
            "method", get_config().get("signals.combination.method", "weighted")
        )
        ...
        self.min_confidence = self.config.get(
            "min_confidence", get_config().get("signals.filtering.min_confidence", 0.6)
        )
        ...
        **kwargs: Any,
        ...
        for generator in self.signal_generators:
            try:
                ...
                logger.exception(
                    "Error generating signals from %s: %s", generator.name, e
                )
...
def simple_strategy(
    data: pd.DataFrame, current_idx: int, params: Dict[str, Any]
) -> Dict[str, Any]:
