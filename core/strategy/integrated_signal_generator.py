...
class IntegratedSignalGenerator:
    ...
    def generate_signal(
        ...
        position_multiplier = self._calculate_position_multiplier(
            combined_signal["confidence"], risk_score, regime_analysis
        )
    ...
    def _get_ml_signal(self, data: pd.DataFrame, current_idx: int) -> Tuple[int, float]:
    ...
    def _get_wave_signal(
        ...
        try:
            ...
            signal_info = self.wave_signal_generator.generate_signal(
                wave_count, data.iloc[current_idx]
            )
    ...
    def _get_sentiment_signal(
        self, market_news: Optional[List[str]], current_time: datetime
    ) -> float:
    ...
    def _combine_signals(
        ...
        sentiment_signal = (
            1 if sentiment_score > 0.2 else (-1 if sentiment_score < -0.2 else 0)
        )
        ...
        if regime_analysis["market_regime"].value in ["trending_up", "trending_down"]:
            regime_weight = (
                self.weights["regime"] * regime_analysis["trend_strength"] / 100
            )
            regime_signal = (
                1 if regime_analysis["market_regime"].value == "trending_up" else -1
            )
        ...
        if total_weight > 0:
            ...
            confidence = min(abs(final_score), 1.0) * (
                total_weight / total_possible_weight if total_possible_weight > 0 else 0
            )
            ...
            # No weights available, return neutral signal
            logger.warning(
                "No signal weights available for %s, returning neutral signal", symbol
            )
            final_score = 0.0
            direction = 0
            confidence = 0.0
    ...
    def _calculate_position_multiplier(
        self, confidence: float, risk_score: float, regime_analysis: Dict
    ) -> float:
    ...
    def _generate_reasoning(
        ...
        if ml_confidence > 0.6:
            ml_dir = (
                "bullish"
                if ml_signal == 1
                else ("bearish" if ml_signal == -1 else "neutral")
            )
        ...
        if wave_pattern and wave_confidence > 0.5:
            reasons.append(
                f"Elliott Wave: {wave_pattern} pattern ({wave_confidence:.1%} confidence)"
            )
        ...
        if combined_signal["direction"] == 0:
            ...
            reasons.append(
                f"Signal: {direction} with {combined_signal['confidence']:.1%} confidence"
            )
