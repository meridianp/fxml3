...
class Position:
    ...
    def __init__(self, position_data: Dict[str, Any]):
        ...
        self.remaining_quantity = Decimal(
            str(position_data.get("remaining_quantity", 0))
        )
        ...
        self.trailing_stop_distance = Decimal(
            str(position_data.get("trailing_stop_distance", 0))
        )
        ...
        # Timing
        self.created_at = position_data.get("created_at", datetime.utcnow())
        self.opened_at = position_data.get("opened_at")
        self.closed_at = position_data.get("closed_at")
        self.last_updated = position_data.get("last_updated", datetime.utcnow())
    ...
    def add_fill(
        ...
        self.avg_entry_price = (
            total_value / self.filled_quantity if self.filled_quantity > 0 else price
        )
        ...
        if (
            ...
            and self.filled_quantity >= self.target_quantity
        ):
...
class PositionManager:
    ...
    async def create_position(self, position_data: Dict[str, Any]) -> Position:
        ...
        async with self._lock:
            ...
            if position.signal_id:
                ...
                self.positions_by_signal[position.signal_id].append(
    ...
    async def update_position_state(
        ...
        async with self._lock:
            ...
            logger.info(
                f"Position {position_id} state changed: {old_state} -> {new_state}"
            )
            return True
    ...
    async def update_stop_loss(self, position_id: str, new_stop: Decimal) -> bool:
        ...
        async with self._lock:
            ...
            logger.info(
                f"Position {position_id} stop loss updated: {old_stop} -> {new_stop}"
            )
            return True

    async def activate_trailing_stop(self, position_id: str, distance: Decimal) -> bool:
        ...
        async with self._lock:
            ...
            logger.info(
                f"Position {position_id} trailing stop activated: distance={distance}"
            )
            return True
    ...
    async def get_position_metrics(self, position_id: str) -> Dict[str, Any]:
        ...
        pnl_percent = (
            (total_pnl / (position.avg_entry_price * position.filled_quantity) * 100)
        ...
        if position.opened_at:
            duration = (position.closed_at or datetime.utcnow()) - position.opened_at
    ...
    def _calculate_risk_reward(self, position: Position) -> float:
        ...
        if position.side == OrderSide.BUY:
            ...
            reward = (
                position.take_profit_1 - position.avg_entry_price
                if position.take_profit_1 > 0
                else 0
            )
        else:
            risk = position.stop_loss - position.avg_entry_price
            reward = (
                position.avg_entry_price - position.take_profit_1
                if position.take_profit_1 > 0
                else 0
            )
