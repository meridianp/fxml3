...
class Trade:
    ...
    def __init__(self, trade_data: Dict[str, Any]):
        ...
        self.trailing_stop_distance = Decimal(
            str(trade_data.get("trailing_stop_distance", 0))
        )
...
class TradeManagerService(BaseService):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.trailing_stop_activation = config.get(
    ...
    async def setup_rabbitmq(self):
        ...
        await self.rabbitmq_channel.declare_queue(Queues.TRADE_MANAGEMENT, durable=True)
    ...
    async def process_trade_execution(self, execution_data: Dict[str, Any]):
        ...
        try:
            ...
            trade_data = {
                ...
                "take_profit_2": execution_data["trade_parameters"].get(
                    "take_profit_2", 0
                ),
                "take_profit_3": execution_data["trade_parameters"].get(
                    "take_profit_3", 0
                ),
                "status": TradeStatus.OPEN,
                "metadata": {
                    "order_id": execution_data["entry_execution"]["client_order_id"],
                    "broker_order_id": execution_data["entry_execution"][
                        "broker_order_id"
                    ],
                    "signal_confidence": execution_data.get("signal_data", {}).get(
                        "confidence", 0
                    ),
                },
            }
            ...
            self.logger.info(
                f"New trade opened: {trade_id} - {trade.symbol} {trade.side}"
            )
        ...
        while self.running:
            try:
                for trade_id, trade in list(self.active_trades.items()):
                    ...
                    trade.unrealized_pnl = self.calculate_unrealized_pnl(
                        trade, current_price
                    )
                    ...
                    exit_signal = (
                        await self.exit_strategy_manager.check_exit_conditions(
                            trade,
                            current_price,
                            self.market_data_1m.get(trade.symbol, []),
                        )
                    )
                    ...
                    partial_exit = await self.check_partial_exit_levels(
                        trade, current_price
                    )
                    if partial_exit:
        ...
        while self.running:
            try:
                ...
                risk_metrics = await self.risk_monitor.calculate_risk_metrics(
                    self.active_trades, self.market_prices
                )
                ...
                if risk_metrics["daily_pnl"] < -self.max_daily_loss:
                    if not self.daily_loss_limit_hit:
                        ...
                        self.logger.warning(
                            f"Daily loss limit hit: {risk_metrics['daily_pnl']:.2%}"
                        )
                        ...
                        await self.send_risk_alert(
                            "Daily loss limit exceeded", risk_metrics
                        )
    ...
    async def execute_exit(self, trade: Trade, exit_signal: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.info(
                f"Executing exit for trade {trade.trade_id}: {exit_signal.get('reason')}"
            )
    ...
    async def execute_partial_exit(self, trade: Trade, partial_exit: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.info(
                f"Executing partial exit for trade {trade.trade_id}: {exit_quantity}"
            )
    ...
    async def activate_trailing_stop(self, trade: Trade, current_price: Decimal):
        ...
        try:
            ...
            self.logger.info(
                f"Trailing stop activated for trade {trade.trade_id} at {new_stop}"
            )
    ...
    async def modify_stop_order(self, trade: Trade, new_stop_price: Decimal):
        ...
        try:
            ...
            if broker_adapter:
                ...
                self.logger.debug(
                    f"Modified stop order for trade {trade.trade_id} to {new_stop_price}"
                )

        except Exception as e:
            self.logger.error(f"Error modifying stop order: {e}")

    async def check_partial_exit_levels(
        self, trade: Trade, current_price: Decimal
        ...
        for i, tp in enumerate(take_profits):
            ...
            if trade.side == "BUY" and current_price >= tp:
                return {
                    ...
                    "target_price": tp,
                    "quantity": trade.position_size
                    * Decimal(str(self.partial_exit_levels[i])),
                }
        ...
        for trade_id, trade in list(self.active_trades.items()):
            if trade.status == TradeStatus.OPEN:
                await self.execute_exit(
                    trade, {"exit_type": "risk_limit", "reason": reason}
                )
    ...
    async def close_oldest_position(self):
        ...
        # Find oldest trade
        oldest_trade = min(self.active_trades.values(), key=lambda t: t.entry_time)

        await self.execute_exit(
            oldest_trade,
            {"exit_type": "position_limit", "reason": "Max positions reached"},
        )
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                """,
                    ...
                    trade.realized_pnl,
                    float(
                        trade.realized_pnl / (trade.entry_price * trade.position_size)
                    )
                    * 100,
                    trade.status.value,
    ...
    async def handle_execution_report(self, execution):
        ...
        try:
            ...
            for trade in self.active_trades.values():
                if (
                    trade.metadata.get("exit_order_id") == execution.client_order_id
                    or trade.metadata.get("order_id") == execution.client_order_id
                ):
                    ...
                    if execution.order_status.value == "FILLED":
                        ...
                        trade.realized_pnl = self.calculate_unrealized_pnl(
                            trade, execution.last_price
                        )
                        trade.status = TradeStatus.CLOSED
                        ...
                        self.logger.info(
                            f"Trade {trade.trade_id} closed via execution report"
                        )
    ...
    async def handle_order_response(self, response):
        ...
        self.logger.info(
            f"Order response: {response.client_order_id} - {response.status}"
        )
                ...
                await self.publish_message(
                    ...
                    ),
                    {
                        ...
                        "closed_trades_today": len(
                            [
                                t
                                for t in self.closed_trades.values()
                                if t.exit_time
